...
class ErrorHandler extends \CErrorHandler
{
    ...
    protected function handleException($exception)
    {
        ...
        // Log MySQL deadlocks
        if ($exception instanceof \CDbException && StringHelper::contains($exception->getMessage(),
                'Deadlock')
        ) {
            // TODO: MySQL specific
            ...
            Craft::error('Deadlock error, innodb status: '.$info,
                'system.db.CDbCommand', __METHOD__);
        }
        ...
        if (isset($trace[3]['object']) && $trace[3]['object'] instanceof \Twig_Template) {
            ...
            foreach ($trace as $step) {
                if (isset($step['class']) && $step['class'] == '\\craft\\app\\services\\Tasks' && $step['function'] == 'runTask') {
                    ...
                    if ($task) {
                        Craft::$app->getTasks()->fail($task,
                            $event->message.' on line '.$event->line.' of '.$event->file);
                    }
    ...
    protected function handleTwigError(\Twig_Error $exception)
    {
        ...
        if (!headers_sent()) {
            HeaderHelper::setHeader("HTTP/1.0 {$data['code']} ".$this->getHttpHeader($data['code'],
                    get_class($exception)));
        }
    ...
    protected function handleDbConnectionError(DbConnectException $exception)
    {
        ...
        if (!headers_sent()) {
            HeaderHelper::setHeader('HTTP/1.0 500 '.$this->getHttpHeader(500,
                    get_class($exception)));
        }
