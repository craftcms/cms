...
final class Entries
{
    ...
    public function moveEntryToSection(Entry $entry, Section $section): bool
    {
        ...
        throw_unless($entry->id, new Exception('Attempting to move an unsaved element.'));

        // and that it's not a nested entry
        throw_if($entry->getPrimaryOwnerId() !== null, new Exception('Attempting to move a nested element.'));
        ...
        throw_if($entry->hasErrors(), new InvalidElementException($entry,
        ...
        $elementsService->ensureBulkOp(function () use (
            ...
            try {
                ...
                throw_unless($elementsService->saveElement($entry, false, false), new InvalidElementException($entry,
