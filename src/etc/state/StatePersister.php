 ...
 *
 * By default, StatePersister stores data in a file named 'state.bin' that is located
 * {@link PathService::getStatePath()}.
 *
 * To retrieve the data from StatePersister, call {@link load()}. To save the data,
 * call {@link save()}.
 *
 ...
 * * Cache: volatile and fast storage. It may be used as storage medium for
 * session or state persister.
 *
 * Since server resource is often limited, be cautious if you plan to use StatePersister
 * to store large amount of data. You should also consider using database-based persister
 * to improve the throughput.
 *
 * StatePersister is a core application component used to store global application state.
 * It may be accessed via {@link WebApp::getStatePersister()}.
 *
 *
 * Craft overrides the default {@link CStatePersister} so it can set a custom path
 * at runtime for our state file.
 *
