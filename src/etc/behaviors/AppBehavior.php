...
class AppBehavior extends BaseBehavior
{
	...
	private function _processConnectionString()
	{
		...
		{
			return 'mysql:host='.craft()->config->get('server', ConfigFile::Db.';dbname=').craft()->config->get('database', ConfigFile::Db).strtolower(';port='.craft()->config->get('port', ConfigFile::Db).';');
		}
	}
