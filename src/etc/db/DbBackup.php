...
class DbBackup
{
	...
	public function restore($filePath)
	{
		...
		// Extract the relevant SQL from the file.
		$statements = $this->_buildSQLStatements($sql);

		// Reconstruct one SQL statement at a time.
	...
	private function _buildSQLStatements($sql)
	{
		...
		{
			// Skip any SQL comments
			if ($statement[0] == '-')
			{
				continue;
			}

			// If the last character is a semicolon, we've found the end of a SQL statement.
			if ($statement[strlen($statement) - 1] == ';')
			{
				// If it's a not a multi-line statement, let's just use what we found.
				if (!$runningStatement)
				{
				...
				{
					// If it is a multi-line SQL statement, we've found the end of it.
