...
class ElementQueryConditionBuilder
{
     ...
     * @return array
     */
    public function extractQueryConditions()
    {
        ...
        $eagerLoadingRules = $this->_traverseAndExtractRules($startingNode);
    ...
    private function _extractArgumentValue(Node $argumentNode, $nodeName = null)
    {
        if (isset($argumentNode->value->values)) {
            ...
            $extractedValue = $argumentNode->kind === 'Variable' ? $this->_resolveInfo->variableValues[$nodeName] : ($argumentNode->value ?? null);
        }
     ...
     * @return array
     */
    private function _traverseAndExtractRules(Node $parentNode, $prefix = '', $context = 'global', $parentField = null)
    {
        $eagerLoadNodes = [];
