 ...
 *
 */
class bSecurityService extends CApplicationComponent
{
	/**
	 * @param $userName
	 * @param $password
	...
	public function validatePTUserCredentialsAndKey($userName, $password, $licenseKeys, $edition)
	{
		...
		$et = new bEt(bEtEndPoints::ValidateKeysByCredentials());
	...
	public function hashPassword($password)
	{
		...
		throw new bException('Could not hash the given password.');
	}
	...
	public function checkPassword($password, $storedHash, $storedEncType)
	{
		...
		if (!$check)
		{
			if (($storedEncType == 'blowfish' && CRYPT_BLOWFISH !== 1) || ($storedEncType == 'extdes' && CRYPT_EXT_DES !== 1))
				throw new bException('This password was encrypted with .'.$storedEncType.', but it appears the server does not support it.  It could have been disabled or was created on a different server.');
