...
class UrlManager extends \yii\web\UrlManager
{
    ...
    {
        $config['showScriptName'] = !\CraftCms\Cms\Craft::generalConfig()->omitScriptNameInUrls;
    ...
    public function createAbsoluteUrl($params, $scheme = null): string
    {
        ...
        $path = \CraftCms\Cms\Craft::generalConfig()->actionTrigger . '/' . $route;
    ...
    private function _getMatchedDiscoverableUrlRoute(Request $request): array|false
    {
        $redirectUri = $request->getPathInfo() === '.well-known/change-password'
            ? \CraftCms\Cms\Craft::generalConfig()->getSetPasswordRequestPath(Craft::$app->getSites()->getCurrentSite()->handle)
            : null;
    ...
    {
        if ($request->getIsSiteRequest() && !\CraftCms\Cms\Craft::generalConfig()->privateTemplateTrigger) {
    ...
    {
        if ($request->getIsSiteRequest() && \CraftCms\Cms\Craft::generalConfig()->headlessMode) {
            return false;
        }
