...
Craft.FieldLayoutDesigner = Garnish.Base.extend(
  {
    ...
    init: function (container, settings) {
      ...
      $('<input/>', {
        type: 'hidden',
        name: this.settings.elementPlacementInputName.replace(
          '[__TAB_NAME__][]',
          ''
        ),
        value: '',
    ...
    renameTab: function ($tab) {
      ...
      if (newName && newName !== oldName) {
        ...
        $tab
          .find('.placement-input')
          .attr('name', this.getElementPlacementInputName(newName));
      }
    },

    promptForTabName: function (oldName) {
      return Craft.escapeHtml(
        prompt(Craft.t('app', 'Give your tab a name.'), oldName)
      );
    },
...
`).appendTo(this.$tabContainer);
    ...
    getElementPlacementInputName: function (tabName) {
      return this.settings.elementPlacementInputName.replace(
        /__TAB_NAME__/g,
        Craft.encodeUriComponent(tabName)
      );
    },
  },
  {
...
Craft.FieldLayoutDesigner.Element = Garnish.Base.extend({
  ...
  init: function (designer, $container) {
    ...
    this.hasCustomWidth =
      this.designer.settings.customizableUi &&
    ...
    if (this.hasSettings) {
      ...
      settingsHtml = settingsHtml
        ? settingsHtml.replace(/\b__ELEMENT_KEY__\b/g, this.key)
        : '';
  ...
  initUi: function () {
    ...
    this.$configInput = $('<input/>', {
      type: 'hidden',
      name: this.designer.settings.elementConfigInputName.replace(
        /\b__ELEMENT_KEY__\b/g,
        this.key
      ),
    }).appendTo(this.$container);
...
<div class="fld-element-settings">
  ...
  <div class="hud-footer">
    <div class="buttons right">
      <button class="btn submit" type="submit">${Craft.t(
        'app',
        'Apply'
      )}</button>
...
<div class="fld-tab fld-insertion" style="height: ${this.$draggee.height()}px;">
  <div class="tabs"><div class="tab sel draggable" style="width: ${$tab.outerWidth()}px; height: ${$tab.outerHeight()}px;"></div></div>
