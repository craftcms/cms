...
const getters = {
  hasMore(state) {
    return state.developersResponseData.currentPage < state.developersResponseData.total
  },
  ...
  getDeveloperIndexParams(state, getters, rootState) {
    return context => {
      ...
      const direction = context.direction ? context.direction : rootState.pluginStore.sortOptions[firstOptionKey]
    ...
    return new Promise((resolve, reject) => {
      ...
      api.searchDevelopers({
        ...
        .then(response => {
          ...
          dispatch('updateDeveloperIndex', {context, response})
            .then(() => {
              resolve(response)
            })
        })
