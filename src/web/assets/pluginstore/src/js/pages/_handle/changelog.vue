...
export default {
  ...
  computed: {
    ...mapState({
      ...
      pluginChangelogPluginId: state => state.pluginStore.pluginChangelogPluginId,
    }),
  ...
  methods: {
    ...
    initPlugin() {
      ...
      if (!this.plugin || (this.plugin && this.plugin.handle !== pluginHandle)) {
        this.loading = true
    ...
    initChangelog() {
      if (
        this.plugin && this.plugin.id
        && !(this.pluginChangelogPluginId && this.pluginChangelogPluginId === this.plugin.id)
      ) {
        this.loading = true
