...
test('Install the plugin', async ({ page, baseURL }) => {
  ...
  await page.waitForResponse(response => response.url().includes('//api.craftcms.com/v1/plugin-store/plugin/seomatic'))
  ...
  await expect(status).toContainText('Updating Composer dependencies (this may take a minute)…');
  await page.waitForResponse(response => response.url().includes('composer-install')) //craft4.test/index.php?p=admin%2Factions%2Fpluginstore%2Finstall%2Fcomposer-install
  await expect(status).toContainText('Installing the plugin');
  await page.waitForResponse(response => response.url().includes('craft-install')) //craft4.test/index.php?p=admin%2Factions%2Fpluginstore%2Finstall%2Fcraft-install
...
test('Uninstall and remove the plugin', async ({ page, baseURL }) => {
  ...
  await expect(status).toContainText('Updating Composer dependencies (this may take a minute)…');
  await page.waitForResponse(response => response.url().includes('composer-remove')) //craft4.test/index.php?p=admin%2Factions%2Fpluginstore%2Fremove%2Fcomposer-remove
