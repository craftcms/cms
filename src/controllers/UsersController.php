...
class UsersController extends BaseController
{
	...
	public function actionSaveAccountSettings()
	{
		...
		if ($userId !== null)
		{
			...
			if (!$user)
				throw new Exception(Blocks::t(TranslationCategory::App, 'No user exists with the Id “{userId}”.', array('{userId}' => $userId)));
		}
		else
			$user = new User();
		...
		if ($userValidates && (!isset($passwordValidates) || $passwordValidates))
		{
			...
			if ($isNewUser && (!blx()->users->getCurrentUser()->admin || blx()->request->getPost('require_verification')))
			{
				...
				blx()->email->sendUserEmailByKey($user, 'verify_email');
			}
