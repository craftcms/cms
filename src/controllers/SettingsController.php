...
class SettingsController extends Controller
{
	...
	public function actionSavePlugin()
	{
		...
		if (b()->request->getPost('disable', null) !== null)
		{
			if (b()->plugins->disable($className))
			{
				b()->user->setMessage(MessageType::Notice, 'Disabled plugin.');
			}
			else
			{
				b()->user->setMessage(MessageType::Error, 'There was a problem disabling the plugin.');
			}
		}
		elseif (b()->request->getPost('enable', null) !== null)
		{
			if (b()->plugins->enable($className))
			{
				b()->user->setMessage(MessageType::Notice, 'Enabled plugin.');
			}
			else
			{
				b()->user->setMessage(MessageType::Error, 'There was a problem enabling the plugin.');
			}
		}
		elseif (b()->request->getPost('install', null) !== null)
		{
			if (b()->plugins->install($className))
			{
				b()->user->setMessage(MessageType::Notice, 'Installed plugin.');
			}
			else
			{
				b()->user->setMessage(MessageType::Error, 'There was a problem installing the plugin.');
			}
		}
		elseif (b()->request->getPost('uninstall', null) !== null)
		{
			if (b()->plugins->uninstall($className))
			{
				b()->user->setMessage(MessageType::Notice, 'Uninstalled plugin.');
			}
			else
			{
				b()->user->setMessage(MessageType::Error, 'There was a problem uninstalling the plugin.');
			}
		}
