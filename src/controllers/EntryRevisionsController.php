...
class EntryRevisionsController extends BaseController
{
	...
	public function actionSaveDraft()
	{
		...
		if (craft()->entryRevisions->saveDraft($draft))
		{
			...
			{
				craft()->deprecator->deprecate('redirect_token_savedraft', 'The {entryId} token within the ‘redirect’ param on entryRevisions/saveDraft requests has been deprecated. Use {id} instead.', '1.1');
	...
	public function actionPublishDraft()
	{
		...
		if (craft()->entryRevisions->publishDraft($draft))
		{
			...
			if (isset($_POST['redirect']) && mb_strpos($_POST['redirect'], '{entryId}') !== false)
			{
				craft()->deprecator->deprecate('redirect_token_publishdraft', 'The {entryId} token within the ‘redirect’ param on entryRevisions/publishDraft requests has been deprecated. Use {id} instead.', '1.1');
				$_POST['redirect'] = str_replace('{entryId}', '{id}', $_POST['redirect']);
			}
