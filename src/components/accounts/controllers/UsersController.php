...
class UsersController extends BaseEntityController
{
	...
	public function actionUploadUserPhoto()
	{
		...
		try
		{
			...
			if ($uploader->file && $uploader->file->getSize())
			{
				...
				$folderPath = blx()->path->getTempPath().'userphotos/'.$user->username.'/';
				...
				if ($width && $height)
				{
					...
					$html = blx()->templates->render('users/_edit/crop_photo_modal',
						array(
	...
	public function actionCropUserPhoto()
	{
		...
		try
		{
			...
			$imagePath = blx()->path->getTempPath().'userphotos/'.$user->username.'/'.$source;
			...
			if (IOHelper::fileExists($imagePath) && blx()->images->setMemoryForImage($imagePath))
			{
				...
				{
					IOHelper::clearFolder(blx()->path->getTempPath().'userphotos/'.$user->username);
