...
final readonly class Drafts
{
    ...
    public function createDraft(
        ...
        throw_if($canonical->getIsDraft() || $canonical->getIsRevision(), InvalidArgumentException::class, 'Cannot create a draft from another draft or revision.');
    ...
    public function applyDraft(ElementInterface $draft, array $newAttributes = []): ElementInterface
    {
        ...
        if ($canonical->siteId !== $draft->siteId) {
            ...
            throw_if($draft === null, Exception::class, "Could not load the draft for site ID $canonical->siteId");
        }
    ...
    public function removeDraftData(ElementInterface $draft): void
    {
        ...
        try {
            ...
            throw_if($draft->hasErrors() || ! Craft::$app->getElements()->saveElement($draft, false, false), InvalidElementException::class, $draft, "Draft $draft->id could not be applied because it doesn't validate.");
