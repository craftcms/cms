...
final class Html
{
    ...
    public static function parseTag(string $tag, int $offset = 0): array
    {
        ...
        throw_if(($htmlEnd = stripos($tag, "</$type>", $cursor)) === false, new InvalidHtmlTagException("Could not find a </$type> tag in string: $tag", $type, $attributes, $start, $htmlStart));
    ...
    public static function parseTagAttribute(string $html, int $offset = 0, ?int &$start = null, ?int &$end = null): ?array
    {
        if (! preg_match('/\s*([^=\/>\s]+)/A', $html, $match, PREG_OFFSET_CAPTURE, $offset)) {
            // No `>`
            throw_unless(preg_match('/(\s*)\/?>/A', $html, $m, 0, $offset), new InvalidArgumentException("Malformed HTML tag attribute in string: $html"));
        ...
        if (preg_match('/\s*=\s*/A', $html, $m, 0, $offset)) {
            ...
            if (isset($html[$offset]) && in_array($html[$offset], ['\'', '"'])) {
                ...
                throw_unless(preg_match("/$q(.*?)$q/sA", $html, $m, 0, $offset), new InvalidArgumentException("Malformed HTML tag attribute in string: $html"));
    ...
    private static function _findTag(string $html, int $offset = 0): array
    {
        ...
        throw_if(! preg_match('/<(\/?[\w\-]+)/', $html, $match, PREG_OFFSET_CAPTURE, $offset) || $match[1][0][0] === '/', new InvalidHtmlTagException(
    ...
    private static function _addToTagInternal(string $tag, string $html, string $position, ?string $ifExists = null): string
    {
        ...
        throw_unless(isset($info['htmlStart']), new InvalidArgumentException("<{$info['type']}> can't have children."));
    ...
    {
        throw_unless(is_file($file), new InvalidArgumentException("Invalid file path: $file"));
        ...
        throw_unless(str_starts_with($file, "$rootPath/"), new InvalidArgumentException(sprintf('%s cannot be passed a path outside of the project root.', __METHOD__)));

        throw_if(Craft::$app->getSecurity()->isSystemDir(dirname($file)), new InvalidArgumentException(sprintf('%s cannot be passed a path within or above system directories.', __METHOD__)));

        $ext = pathinfo($file, PATHINFO_EXTENSION);
        throw_if(strtolower($ext) === 'php', new InvalidArgumentException(sprintf('%s cannot be passed a path to a PHP file.', __METHOD__)));
    ...
    public static function svg(
        ...
        if ($svg instanceof Asset) {
            ...
            if (! is_file($svg) || ! FileHelper::isSvg($svg)) {
                throw_if($throwException, new InvalidArgumentException("Invalid SVG path: $svg"));
