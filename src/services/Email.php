...
class Email extends Component
{
    ...
    public function sendEmailByKey(User $user, $key, $variables = [])
    {
        ...
        if (Craft::$app->getEdition() >= Craft::Client) {
            $message = Craft::$app->getEmailMessages()->getMessage($key,
                $user->getPreferredLocale());

            $emailModel->subject = $message->subject;
            $emailModel->body = $message->body;
        } else {
            $emailModel->subject = Craft::t('app', $key.'_subject', null,
                'en_us');
    ...
    public function sendTestEmail($settings)
    {
        ...
        $success = $this->sendEmailByKey($user, 'test_email',
        ...
        if ($event->performAction) {
            ...
            switch ($emailSettings['protocol']) {
                ...
                case EmailerType::Pop: {
                    ...
                    $pop->authorize($emailSettings['host'],
                        $emailSettings['port'], $emailSettings['timeout'],
                        $emailSettings['username'], $emailSettings['password'],
                        Craft::$app->getConfig()->get('devMode') ? 1 : 0);
            ...
            if (!empty($emailModel->stringAttachments)) {
                foreach ($emailModel->stringAttachments as $stringAttachment) {
                    $email->addStringAttachment($stringAttachment['string'],
                        $stringAttachment['filename'],
                        $stringAttachment['encoding'],
            ...
            if (!empty($emailModel->attachments)) {
                foreach ($emailModel->attachments as $attachment) {
                    $email->addAttachment($attachment['path'],
                        $attachment['name'], $attachment['encoding'],
                        $attachment['type']);
                }
            }
            ...
            if ($emailModel->htmlBody) {
                $renderedHtmlBody = Craft::$app->getView()->renderString($emailModel->htmlBody,
                    $variables);
                ...
                $renderedHtmlBody = Craft::$app->getView()->renderString(Markdown::process($emailModel->body),
                    $variables);
                $email->msgHTML($renderedHtmlBody);
                $email->AltBody = Craft::$app->getView()->renderString($emailModel->body,
                    $variables);
