...
class TemplatesService extends BaseApplicationComponent
{
	...
	public function namespaceInputs($html, $namespace = null, $otherAttributes = true)
	{
		...
		if ($namespace)
		{
			...
			if ($otherAttributes)
			{
				...
				$html = preg_replace('/(?<![\w\-])((id=|for=|data\-(reverse\-)?target=|data-target-prefix=)(\'|"))([^\'"]+)\3/i', '$1'.$idNamespace.'-$4$3', $html);
			}
	...
	private function _getCpElementHtml(&$context)
	{
		...
		$html .= '" data-id="'.$context['element']->id.'" data-locale="'.$context['element']->locale.'" data-status="'.$context['element']->getStatus().'" data-url="'.$context['element']->getUrl().'"';
