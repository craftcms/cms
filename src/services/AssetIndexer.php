...
class AssetIndexer extends Component
{
    ...
    public function storeIndexList(Generator $indexList, int $sessionId, Volume $volume): int
    {
        ...
        foreach ($indexList as $volumeListing) {
            $values[] = [
                $volume->id,
                $sessionId,
                $volumeListing->getAdjustedUri($fsSubpath),
     ...
     * @since 4.0.0
     */
    public function indexFileByListing(Volume $volume, FsListing $listing, int $sessionId, bool $cacheImages = false, bool $createIfMissing = true): Asset
    {
     ...
     * @since 4.0.0
     */
    public function indexFolderByListing(Volume $volume, FsListing $listing, int $sessionId, bool $createIfMissing = true): VolumeFolder
    {
        $indexEntry = new AssetIndexData([
            'volumeId' => $volume->id,
            'sessionId' => $sessionId,
    ...
    public function indexFileByEntry(AssetIndexData $indexEntry, bool $cacheImages = false, bool $createIfMissing = true): Asset
    {
        ...
        if (!in_array(strtolower($extension), \CraftCms\Cms\Craft::generalConfig()->allowedFileExtensions, true)) {
        ...
        try {
            ...
            if ($asset->kind === Asset::KIND_IMAGE) {
                ...
                $shouldCache = !$fs instanceof LocalFsInterface && $cacheImages && \CraftCms\Cms\Craft::generalConfig()->maxCachedCloudImageSize > 0;
