	...
	public function deleteEntry($entries)
	{
		...
		try
		{
			...
			foreach ($entries as $entry)
			{
				...
				// Does this entry live in a Structure section?
				if ($entry->getSection()->type == SectionType::Structure)
				{
					// Delete the row in craft_entries via StructuredEntryRecord, so NestedSetBehavior can do its thing
					$entryRecord = StructuredEntryRecord::model()->findById($entry->id);

					if ($entryRecord)
					{
						$entryRecord->deleteNode();
					}
				}
			}
