...
this.$draggee.css("display");this.createHelpers();this.settings.removeDraggee?this.$draggee.hide():this.$draggee.css("visibility","hidden");this.lastMouseX=this.lastMouseY=null;this.otherItems=[];for(var a=0;a<this.$items.length;a++){var b=this.$items[a];-1==c.inArray(b,this.$draggee)&&this.otherItems.push(b)}this.totalOtherItems=this.otherItems.length;this.helperLagIncrement=1==this.helpers.length?0:Garnish.Drag.helperLagIncrementDividend/(this.helpers.length-1);this.updateHelperPosInterval=setInterval(c.proxy(this,
"updateHelperPos"),Garnish.Drag.updateHelperPosInterval);a=this.$draggee.offset();this.draggeeMouseOffsetX=this.mouseX-(a.left+this.$draggee.outerWidth()/2);this.draggeeMouseOffsetY=this.mouseY-(a.top+this.$draggee.outerHeight()/2);this.base()},onDragStop:function(){clearInterval(this.updateHelperPosInterval);this.base()},createHelpers:function(){for(var a=0;a<this.$draggee.length;a++){var b=c(this.$draggee[a]),d=b.clone().addClass("draghelper");d.css({width:b.width(),height:b.height(),margin:0});
...
this.targetItemMouseDiffY+a*Garnish.Drag.helperSpacingY}},updateHelperPos:function(){if(this.mouseX!==this.lastMouseX||this.mouseY!==this.lastMouseY){for(this.updateHelperPos._i=0;this.updateHelperPos._i<this.helpers.length;this.updateHelperPos._i++)this.helperTargets[this.updateHelperPos._i]=this.getHelperTarget(this.updateHelperPos._i);this.lastMouseX=this.mouseX;this.lastMouseY=this.mouseY}for(this.updateHelperPos._j=0;this.updateHelperPos._j<this.helpers.length;this.updateHelperPos._j++)this.updateHelperPos._lag=
...
returnHelpersToDraggees:function(){for(var a=0;a<this.$draggee.length;a++){var b=c(this.$draggee[a]),d=this.helpers[a],e=b.offset();(function(a,b){b.velocity({left:e.left,top:e.top},"fast",function(){a.css("visibility","inherit");b.remove()})})(b,d)}}},{helperZindex:1E3,helperLagBase:1,helperLagIncrementDividend:1.5,updateHelperPosInterval:20,helperSpacingX:5,helperSpacingY:5,defaults:{removeDraggee:!1,helperOpacity:1,helper:null}});Garnish.DragDrop=Garnish.Drag.extend({$dropTargets:null,$activeDropTarget:null,
...
Garnish.Menu.defaults);this.$container=c(a);this.$options=c();this.addOptions(this.$container.find("a"));this.settings.attachToElement&&(this.$trigger=c(this.settings.attachToElement));this.addListener(this.$container,"mousedown",function(a){a.stopPropagation()})},addOptions:function(a){this.$options=this.$options.add(a);a.data("menu",this);this.addListener(a,"click","selectOption")},setPositionRelativeToTrigger:function(){var a=Garnish.$win.height(),b=Garnish.$win.scrollTop(),c=this.$trigger.offset(),
e=this.$trigger.outerWidth(),f=this.$trigger.outerHeight(),f=c.top+f,g=c.top,k=this.$container.outerHeight(),a=a+b-f,h=g-b,b={minWidth:e-(this.$container.outerWidth()-this.$container.width())};b.top=a>=k||a>=h?f:g-k;(f=this.$container.data("align"))||(f="left");Garnish.rtl&&("left"==f?f="right":"right"==f&&(f="left"));switch(f){case "right":b.right=Garnish.$win.width()-(c.left+e);break;case "center":b.left=Math.round(c.left+e/2-this.$container.outerWidth()/2);break;case "left":b.left=c.left}this.$container.css(b)},
...
this.$container&&(this.$shade.appendTo(Garnish.$bod),this.$container.appendTo(Garnish.$bod),this.$container.show(),this.updateSizeAndPosition(),this.$shade.fadeIn(50),this.$container.delay(50).fadeIn(c.proxy(this,"onFadeIn")),this.settings.hideOnShadeClick&&this.addListener(this.$shade,"click","hide"),this.addListener(Garnish.$win,"resize","updateSizeAndPosition"));this.settings.hideOnEsc&&Garnish.escManager.register(this,"hide");this.visible||(this.visible=!0,Garnish.Modal.visibleModal=this,this.settings.onShow())},
quickShow:function(){this.show();this.$container&&(this.$container.finish(),this.$shade.finish())},hide:function(a){a&&a.stopPropagation();this.$container&&(this.$container.fadeOut("fast"),this.$shade.fadeOut("fast",c.proxy(this,"onFadeOut")),this.settings.hideOnShadeClick&&this.removeListener(this.$shade,"click"),this.removeListener(Garnish.$win,"resize"));this.visible=!1;Garnish.Modal.visibleModal=null;this.settings.hideOnEsc&&Garnish.escManager.unregister(this);this.settings.onHide()},quickHide:function(){this.hide();
this.$container&&(this.$container.finish(),this.$shade.finish())},updateSizeAndPosition:function(){this.$container&&(this.$container.css({width:this.desiredWidth?Math.max(this.desiredWidth,200):"",height:this.desiredHeight?Math.max(this.desiredHeight,200):"","min-width":"","min-height":""}),this.updateSizeAndPosition._windowWidth=Garnish.$win.width(),this.updateSizeAndPosition._width=Math.min(this.getWidth(),this.updateSizeAndPosition._windowWidth-20),this.$container.css({width:this.updateSizeAndPosition._width,
...
c();this.addItems(b);this.addListener(this.$container,"click",function(a){this.ignoreClick?this.ignoreClick=!1:this.deselectAll(!0)})},getItemIndex:function(a){return this.$items.index(a[0])},isSelected:function(a){return a.hasClass(this.settings.selectedClass)},selectItem:function(a){this.settings.multi||this.deselectAll();a.addClass(this.settings.selectedClass);this.$first=this.$last=a;this.first=this.last=this.getItemIndex(a);this.setFocusableItem(a);a.focus();this.totalSelected++;this.setCallbackTimeout()},
selectAll:function(){this.settings.multi&&this.$items.length&&(this.first=0,this.last=this.$items.length-1,this.$first=c(this.$items[this.first]),this.$last=c(this.$items[this.last]),this.$items.addClass(this.settings.selectedClass),this.totalSelected=this.$items.length,this.setCallbackTimeout())},selectRange:function(a){if(!this.settings.multi)return this.selectItem(a);this.deselectAll();this.$last=a;this.last=this.getItemIndex(a);this.setFocusableItem(a);a.focus();if(this.first<this.last){a=this.first;
var b=this.last+1}else a=this.last,b=this.first+1;this.$items.slice(a,b).addClass(this.settings.selectedClass);this.totalSelected=b-a;this.setCallbackTimeout()},deselectItem:function(a){a.removeClass(this.settings.selectedClass);a=this.getItemIndex(a);this.first===a&&(this.$first=this.first=null);this.last===a&&(this.$last=this.last=null);this.totalSelected--;this.setCallbackTimeout()},deselectAll:function(a){this.$items.removeClass(this.settings.selectedClass);a&&(this.$first=this.first=this.$last=
this.last=null);this.totalSelected=0;this.setCallbackTimeout()},deselectOthers:function(a){this.deselectAll();this.selectItem(a)},toggleItem:function(a){this.isSelected(a)?this.deselectItem(a):this.selectItem(a)},clearMouseUpTimeout:function(){clearTimeout(this.mouseUpTimeout)},onMouseDown:function(a){if(a.which==Garnish.PRIMARY_CLICK&&(!this.settings.filter||c(a.target).is(this.settings.filter))){this.mousedownX=a.pageX;this.mousedownY=a.pageY;var b=c(c.data(a.currentTarget,"select-item"));a.metaKey||
a.ctrlKey?this.toggleItem(b):null!==this.first&&a.shiftKey&&this.selectRange(b)}},onMouseUp:function(a){if(a.which==Garnish.PRIMARY_CLICK&&(!this.settings.filter||c(a.target).is(this.settings.filter))){var b=c(c.data(a.currentTarget,"select-item"));!a.metaKey&&!a.ctrlKey&&!a.shiftKey&&1>Garnish.getDist(this.mousedownX,this.mousedownY,a.pageX,a.pageY)&&(this.isSelected(b)?(this.clearMouseUpTimeout(),this.mouseUpTimeout=setTimeout(c.proxy(function(){this.deselectOthers(b)},this),300)):(this.deselectAll(),
