...
arguments.length){var c=this[a];if(c&&"function"==typeof b&&(!c.valueOf||c.valueOf()!=b.valueOf())&&/\bbase\b/.test(b)){var e=b.valueOf();b=function(){var a=this.base||h.prototype.base;this.base=c;var b=e.apply(this,arguments);this.base=a;return b};b.valueOf=function(a){return"object"==a?b:e};b.toString=h.toString}this[a]=b}else if(a){var f=h.prototype.extend;h._prototyping||"function"==typeof this||(f=this.extend||f);for(var g={toSource:null},p=["constructor","toString","valueOf"],l=h._prototyping?
...
b){var d=c(a),e=c(b),f=d.scrollTop(),g=e.offset().top,g=d[0]==window?g-f:g-d.offset().top;if(0>g)d.scrollTop(f+g);else{var e=e.outerHeight(),p=d[0]==window?window.innerHeight:d[0].clientHeight;g+e>p&&d.scrollTop(f+(g-(p-e)))}},SHAKE_STEPS:10,SHAKE_STEP_DURATION:25,shake:function(a,b){var d=c(a);b||(b="margin-left");var e=parseInt(d.css(b));isNaN(e)&&(e=0);for(var f=0;f<=Garnish.SHAKE_STEPS;f++)(function(a){setTimeout(function(){Garnish.shake._properties={};Garnish.shake._properties[b]=e+(a%2?-1:1)*
...
b.data("garnish-resizable",!0);var e=a.offsetWidth,f=a.offsetHeight,g=d[0].firstElementChild.firstChild,h=d[0].lastElementChild.firstChild,q=function(c){if(e!=a.offsetWidth||f!=a.offsetHeight)e=a.offsetWidth,f=a.offsetHeight,m(),c&&c.currentTarget==b[0]&&"resize"==c.type||b.trigger("resize")},m=function(){g.style.width=e-1+"px";g.style.height=f-1+"px";h.style.width=e+1+"px";h.style.height=f+1+"px"};m();b.on("resize",q);Garnish.$win.on("resize",q);r(d[0],"over",q);r(d[0],"under",q);r(d[0].firstElementChild,
...
this.$container.find("a");this.$options.data("menu",this);this.settings.attachToElement&&(this.$trigger=c(this.settings.attachToElement));this.addListener(this.$container,"mousedown",function(a){a.stopPropagation()});this.addListener(this.$options,"click","selectOption")},setPositionRelativeToTrigger:function(){var a=Garnish.$win.height(),b=Garnish.$win.scrollTop(),c=this.$trigger.offset(),e=this.$trigger.outerWidth(),f=this.$trigger.outerHeight(),f=c.top+f,g=c.top,h=this.$container.outerHeight(),
a=a+b-f,l=g-b,b={minWidth:e-(this.$container.outerWidth()-this.$container.width())};b.top=a>=h||a>=l?f:g-h;(f=this.$container.data("align"))||(f="left");Garnish.rtl&&("left"==f?f="right":"right"==f&&(f="left"));switch(f){case "right":b.right=Garnish.$win.width()-(c.left+e);break;case "center":b.left=Math.round(c.left+e/2-this.$container.outerWidth()/2);break;case "left":b.left=c.left}this.$container.css(b)},show:function(){this.$trigger&&this.setPositionRelativeToTrigger();this.$container.fadeIn(50);
...
this.setFocus(a),this.setCarotPos(a,0)):this.addTextElement()},addTextElement:function(a){var b=new m(this);this.addElement(b.$input,a);return b},addElement:function(a,b){if("undefined"==typeof b)if(this.focussedElement){var d=this.focussedElement,e=this.getElementIndex(d);if(this.isText(d)){var f=d.prop("selectionStart"),g=d.prop("selectionEnd"),h=d.val(),f=h.substring(0,f),g=h.substr(g);f&&g?(d.val(f).trigger("change"),d=new m(this),d.$input.val(g).trigger("change"),this.addElement(d.$input,e+1),
...
this.isText(a))){var b=a.val().length;this.setCarotPos(a,b)}},focusNextElement:function(a){a=this.getElementIndex(a);a<this.elements.length-1&&(a=this.elements[a+1],this.setFocus(a),this.isText(a)&&this.setCarotPos(a,0))},setCarotPos:function(a,b){a.prop("selectionStart",b);a.prop("selectionEnd",b)}});var m=Garnish.Base.extend({parentInput:null,$input:null,$stage:null,val:null,focussed:!1,interval:null,init:function(a){this.parentInput=a;this.$input=c('<input type="text"/>').appendTo(this.parentInput.$container);
this.$input.css("margin-right",2-m.padding+"px");this.setWidth();this.addListener(this.$input,"focus","onFocus");this.addListener(this.$input,"blur","onBlur");this.addListener(this.$input,"keydown","onKeyDown");this.addListener(this.$input,"change","checkInput")},getIndex:function(){return this.parentInput.getElementIndex(this.$input)},buildStage:function(){this.$stage=c("<stage/>").appendTo(Garnish.$bod);this.$stage.css({position:"absolute",top:-9999,left:-9999,wordWrap:"nowrap"});Garnish.copyTextStyles(this.$input,
...
typeof this.$items[a];a+=b){var m=c(this.$items[a]),n=m.offset();if(d.isNextRow(n[e.rowOffset],g[e.rowOffset])){if(null===h)h=n[e.rowOffset];else if(n[e.rowOffset]!=h)break;n=n[e.midpointOffset]+Math.round(m[e.midpointSizeFunc]()/2);n=Math.abs(f-n);if(null===l||n<l)l=n,k=m;else break}else if(d.isWrongDirection(n[e.rowOffset],g[e.rowOffset]))break}return k},getFurthestItemToTheLeft:function(a){return this.getFurthestItem(a,"ToTheLeft")},getFurthestItemToTheRight:function(a){return this.getFurthestItem(a,
