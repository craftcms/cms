...
this.elements=[];this.$container.attr("tabindex",0);this.addListener(this.$container,"focus","onFocus")},getElementIndex:function(a){return c.inArray(a,this.elements)},isText:function(a){return"INPUT"==a.prop("nodeName")},onFocus:function(a){this.elements.length?(a=this.elements[0],this.setFocus(a),this.setCarotPos(a,0)):this.addTextElement()},addTextElement:function(a){var b=new p(this);this.addElement(b.$input,a);return b},addElement:function(a,b){if("undefined"==typeof b)if(this.focussedElement){var d=
...
c('<div class="modal-shade"/>').appendTo(Garnish.$bod);a&&(this.setContainer(a),this.show());Garnish.Modal.instances.push(this)},setContainer:function(a){this.$container=c(a);this.$container.data("modal")&&(Garnish.log("Double-instantiating a modal on an element"),this.$container.data("modal").destroy());this.$container.data("modal",this);this.$header=this.$container.find(".pane-head:first");this.$body=this.$container.find(".pane-body:first");this.$scrollpane=this.$body.children(".scrollpane:first");
this.$footer=this.$container.find(".pane-foot:first");this.$footerBtns=this.$footer.find(".btn");this.$submitBtn=this.$footerBtns.filter(".submit:first");this.$closeBtn=this.$footerBtns.filter(".close:first");this.settings.draggable&&this.$header.add(this.$footer).length&&(this.dragger=new Garnish.DragMove(this.$container,{handle:this.$container}));this.addListener(this.$container,"click",function(a){a.stopPropagation()});this.addListener(this.$container,"keydown","onKeyDown");this.addListener(this.$closeBtn,
"click","hide")},show:function(){Garnish.Modal.visibleModal&&this.settings.closeOtherModals&&Garnish.Modal.visibleModal.hide();if(this.$container){this.$container.show();var a=this.getHeight();this.$container.css("margin-top",-Math.round(a/2));a=Garnish.$win.width();this.$container.width()>a&&this.$container.css({width:a,marginLeft:-Math.round(a/2)});this.$container.delay(50).fadeIn(c.proxy(this,"onFadeIn"))}this.visible=!0;Garnish.Modal.visibleModal=this;this.$shade.fadeIn(50);this.addListener(this.$shade,
"click","hide");Garnish.escManager.register(this,"hide");this.settings.onShow()},hide:function(a){a&&a.stopPropagation();this.$container&&(this.$container.fadeOut("fast"),this.removeListener(Garnish.$win,"resize"));this.visible=!1;Garnish.Modal.visibleModal=null;this.$shade.fadeOut("fast",c.proxy(this,"onFadeOut"));this.removeListener(this.$shade,"click");this.removeListener(Garnish.$bod,"keyup");Garnish.escManager.unregister(this);this.settings.onHide()},onFadeIn:function(){this.settings.onFadeIn()},
onFadeOut:function(){this.settings.onFadeOut()},getHeight:function(){if(!this.$container)throw"Attempted to get the height of a modal whose container has not been set.";this.visible||this.$container.show();var a=this.$container.outerHeight();this.visible||this.$container.hide();return a},getWidth:function(){if(!this.$container)throw"Attempted to get the width of a modal whose container has not been set.";this.visible||this.$container.show();var a=this.$container.outerWidth();this.visible||this.$container.hide();
return a},positionRelativeTo:function(a){if(!this.$container)throw"Attempted to position a modal whose container has not been set.";var b=c(a);a=b.offset();var d=Garnish.$bod.scrollTop(),d=a.top-d,e=this.getHeight(),b=e<d+Garnish.navHeight+2*Garnish.Modal.relativeElemPadding?a.top-e-Garnish.Modal.relativeElemPadding:a.top+b.height()+Garnish.Modal.relativeElemPadding;this.$container.css({top:b,left:a.left})},onKeyDown:function(a){"TEXTAREA"!=a.target.nodeName&&a.keyCode==Garnish.RETURN_KEY&&this.$submitBtn.click()},
...
interval:null,initialized:!1,init:function(a,b){this.$input=c(a);this.settings=c.extend({},Garnish.NiceText.defaults,b);this.$input.data("nicetext")&&(Garnish.log("Double-instantiating a transparent text input on an element"),this.$input.data("nicetext").destroy());this.$input.data("nicetext",this);this.getVal();if(this.autoHeight=this.settings.autoHeight&&"TEXTAREA"==this.$input.prop("nodeName"))this.minHeight=this.getHeightForValue(""),this.updateHeight(),this.addListener(Garnish.$win,"resize",
"updateHeight");this.settings.hint&&(this.$hintContainer=c('<div class="texthint-container"/>').insertBefore(this.$input),this.$hint=c('<div class="texthint">'+this.settings.hint+"</div>").appendTo(this.$hintContainer),this.$hint.css({top:parseInt(this.$input.css("borderTopWidth"))+parseInt(this.$input.css("paddingTop")),left:parseInt(this.$input.css("borderLeftWidth"))+parseInt(this.$input.css("paddingLeft"))+1}),Garnish.copyTextStyles(this.$input,this.$hint),this.val?this.$hint.hide():this.showingHint=
!0,this.addListener(this.$hint,"mousedown",function(a){a.preventDefault();this.$input.focus()}));this.addListener(this.$input,"focus","onFocus");this.addListener(this.$input,"blur","onBlur");this.addListener(this.$input,"keydown","onKeyDown");this.initialized=!0},getVal:function(){return this.val=this.$input.val()},showHint:function(){this.$hint.fadeIn(Garnish.NiceText.hintFadeDuration);this.showingHint=!0},hideHint:function(){this.$hint.fadeOut(Garnish.NiceText.hintFadeDuration);this.showingHint=
!1},checkInput:function(){var a=this.val!==this.getVal();a&&(this.$hint&&(this.showingHint&&this.val?this.hideHint():this.showingHint||this.val||this.showHint()),this.autoHeight&&this.updateHeight());return a},buildStage:function(){this.$stage=c("<stage/>").appendTo(Garnish.$bod);this.$stage.css({display:"block",position:"absolute",top:-9999,left:-9999,wordWrap:"break-word"});this.inputBoxSizing=this.$input.css("box-sizing");"border-box"==this.inputBoxSizing&&this.$stage.css({"border-top":this.$input.css("border-top"),
