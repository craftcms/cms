...
c(a);this.settings=c.extend({},Garnish.NiceText.defaults,b);if(this.maxLength=this.$input.attr("maxlength"))this.maxLength=parseInt(this.maxLength);this.maxLength&&(this.settings.showCharsLeft||Garnish.hasAttr(this.$input,"data-show-chars-left"))&&(this.showCharsLeft=!0,this.$input.removeAttr("maxlength"));this.$input.data("nicetext")&&(Garnish.log("Double-instantiating a transparent text input on an element"),this.$input.data("nicetext").destroy());this.$input.data("nicetext",this);this.getVal();
(this.autoHeight=this.settings.autoHeight&&"TEXTAREA"==this.$input.prop("nodeName"))&&(this.$input.height()?this.initAutoHeight():this.addListener(Garnish.$win,"resize","initAutoHeightIfVisible"));this.settings.hint&&(this.$hintContainer=c('<div class="texthint-container"/>').insertBefore(this.$input),this.$hint=c('<div class="texthint">'+this.settings.hint+"</div>").appendTo(this.$hintContainer),this.$hint.css({top:parseInt(this.$input.css("borderTopWidth"))+parseInt(this.$input.css("paddingTop")),
