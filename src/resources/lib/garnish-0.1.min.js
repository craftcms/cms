...
c(a);this.settings=c.extend({},Garnish.NiceText.defaults,b);this.initIfVisible()||this.addListener(Garnish.$win,"resize","initIfVisible")},initIfVisible:function(){if(0<this.$input.height()){this.removeListener(Garnish.$win,"resize");if(this.maxLength=this.$input.attr("maxlength"))this.maxLength=parseInt(this.maxLength);this.maxLength&&(this.settings.showCharsLeft||Garnish.hasAttr(this.$input,"data-show-chars-left"))&&(this.showCharsLeft=!0,this.$input.removeAttr("maxlength"));this.$input.data("nicetext")&&
(Garnish.log("Double-instantiating a transparent text input on an element"),this.$input.data("nicetext").destroy());this.$input.data("nicetext",this);this.getVal();if(this.autoHeight=this.settings.autoHeight&&"TEXTAREA"==this.$input.prop("nodeName"))this.minHeight=this.getHeightForValue(""),this.updateHeight(),this.width=this.$input.width(),this.addListener(Garnish.$win,"resize","updateHeightIfWidthChanged");this.settings.hint&&(this.$hintContainer=c('<div class="texthint-container"/>').insertBefore(this.$input),
...
case Garnish.LEFT_KEY:Garnish.ltr?this.selectPrev():this.selectNext(),a.preventDefault()}},destroy:function(){this.$outerContainer.removeData("pill");this.base()}});Garnish.Select=Garnish.Base.extend({$container:null,$items:null,$selectedItems:null,mousedownX:null,mousedownY:null,mouseUpTimeout:null,callbackFrame:null,$focusable:null,$first:null,first:null,$last:null,last:null,init:function(a,b,e){this.$container=c(a);"undefined"==typeof b&&c.isPlainObject(a)?(e=a,b=a=null):"undefined"==typeof e&&
c.isPlainObject(b)&&(e=b,b=null);this.$container.data("select")&&(Garnish.log("Double-instantiating a select on an element"),this.$container.data("select").destroy());this.$container.data("select",this);this.setSettings(e,Garnish.Select.defaults);this.$items=c();this.$selectedItems=c();this.addItems(b);this.settings.allowEmpty&&!this.settings.checkboxMode&&this.addListener(this.$container,"click",function(a){this.ignoreClick?this.ignoreClick=!1:this.deselectAll(!0)})},getItemIndex:function(a){return this.$items.index(a[0])},
isSelected:function(a){if(Garnish.isJquery(a)){if(!a[0])return!1;a=a[0]}return-1!=c.inArray(a,this.$selectedItems)},selectItem:function(a,b,c){this.settings.multi||this.deselectAll();this.$first=this.$last=a;this.first=this.last=this.getItemIndex(a);b&&(this.setFocusableItem(a),this.focusItem(a,c));this._selectItems(a)},selectAll:function(){this.settings.multi&&this.$items.length&&(this.first=0,this.last=this.$items.length-1,this.$first=this.$items.eq(this.first),this.$last=this.$items.eq(this.last),
this._selectItems(this.$items))},selectRange:function(a,b){if(!this.settings.multi)return this.selectItem(a,!0);this.deselectAll();this.$last=a;this.last=this.getItemIndex(a);this.setFocusableItem(a);this.focusItem(a,b);if(this.first<this.last)var c=this.first,d=this.last+1;else c=this.last,d=this.first+1;this._selectItems(this.$items.slice(c,d))},deselectItem:function(a){var b=this.getItemIndex(a);this.first===b&&(this.$first=this.first=null);this.last===b&&(this.$last=this.last=null);this._deselectItems(a)},
deselectAll:function(a){a&&(this.$first=this.first=this.$last=this.last=null);this._deselectItems(this.$items)},deselectOthers:function(a){this.deselectAll();this.selectItem(a,!0)},toggleItem:function(a,b){this.isSelected(a)?this._canDeselect(a)&&this.deselectItem(a,!0):this.selectItem(a,!0,b)},clearMouseUpTimeout:function(){clearTimeout(this.mouseUpTimeout)},getFirstItem:function(){if(this.$items.length)return this.$items.first()},getLastItem:function(){if(this.$items.length)return this.$items.last()},
isPreviousItem:function(a){return 0<a},isNextItem:function(a){return a<this.$items.length-1},getPreviousItem:function(a){if(this.isPreviousItem(a))return this.$items.eq(a-1)},getNextItem:function(a){if(this.isNextItem(a))return this.$items.eq(a+1)},getItemToTheLeft:function(a){var b=Garnish.ltr?"Previous":"Next";if(this["is"+b+"Item"](a)){if(this.settings.horizontal)return this["get"+b+"Item"](a);if(!this.settings.vertical)return this.getClosestItem(a,Garnish.X_AXIS,"<")}},getItemToTheRight:function(a){var b=
...
Garnish.Y_AXIS,">")}},getClosestItem:function(a,b,c){var d=Garnish.Select.closestItemAxisProps[b];c=Garnish.Select.closestItemDirectionProps[c];var f=this.$items.eq(a),g=f.offset(),f=g[d.midpointOffset]+Math.round(f[d.midpointSizeFunc]()/2),k=null,h=null,l=null;b=Garnish.rtl&&b==Garnish.X_AXIS?-1*c.step:c.step;for(a+=b;"undefined"!=typeof this.$items[a];a+=b){var p=this.$items.eq(a),m=p.offset();if(c.isNextRow(m[d.rowOffset],g[d.rowOffset])){if(null===k)k=m[d.rowOffset];else if(m[d.rowOffset]!=k)break;
m=m[d.midpointOffset]+Math.round(p[d.midpointSizeFunc]()/2);m=Math.abs(f-m);if(null===h||m<h)h=m,l=p;else break}else if(c.isWrongDirection(m[d.rowOffset],g[d.rowOffset]))break}return l},getFurthestItemToTheLeft:function(a){return this.getFurthestItem(a,"ToTheLeft")},getFurthestItemToTheRight:function(a){return this.getFurthestItem(a,"ToTheRight")},getFurthestItemAbove:function(a){return this.getFurthestItem(a,"Above")},getFurthestItemBelow:function(a){return this.getFurthestItem(a,"Below")},getFurthestItem:function(a,
b){for(var c,d;d=this["getItem"+b](a);)c=d,a=this.getItemIndex(c);return c},get totalSelected(){return this.getTotalSelected()},getTotalSelected:function(){return this.$selectedItems.length},addItems:function(a){a=c(a);for(var b=0;b<a.length;b++){var e=a[b];c.data(e,"select")&&(Garnish.log("Element was added to more than one selector"),c.data(e,"select").removeItems(e));c.data(e,"select",this);if(this.settings.handle)if("object"==typeof this.settings.handle)var d=c(this.settings.handle);else"string"==
typeof this.settings.handle?d=c(e).find(this.settings.handle):"function"==typeof this.settings.handle&&(d=c(this.settings.handle(e)));else d=c(e);c.data(e,"select-handle",d);d.data("select-item",e);this.addListener(d,"mousedown","onMouseDown");this.addListener(d,"mouseup","onMouseUp");this.addListener(d,"click",function(a){this.ignoreClick=!0});this.addListener(e,"keydown","onKeyDown")}this.$items=this.$items.add(a);this.updateIndexes()},removeItems:function(a){a=c.makeArray(a);for(var b=!1,e=!1,
d=0;d<a.length;d++){var f=a[d],g=c.inArray(f,this.$items);-1!=g&&(this._deinitItem(f),this.$items.splice(g,1),b=!0,f=c.inArray(f,this.$selectedItems),-1!=f&&(this.$selectedItems.splice(f,1),e=!0))}b&&(this.updateIndexes(),e&&(c(a).removeClass(this.settings.selectedClass),this.onSelectionChange()))},removeAllItems:function(){for(var a=0;a<this.$items.length;a++)this._deinitItem(this.$items[a]);this.$items=c();this.$selectedItems=c();this.updateIndexes()},updateIndexes:function(){null!==this.first?
(this.first=this.getItemIndex(this.$first),this.setFocusableItem(this.$first)):this.$items.length&&this.setFocusableItem(c(this.$items[0]));null!==this.last&&(this.last=this.getItemIndex(this.$last))},resetItemOrder:function(){this.$items=c().add(this.$items);this.$selectedItems=c().add(this.$selectedItems);this.updateIndexes()},setFocusableItem:function(a){this.$focusable&&this.$focusable.removeAttr("tabindex");this.$focusable=a.attr("tabindex","0")},focusItem:function(a,b){if(b){var c=Garnish.$doc.scrollLeft(),
d=Garnish.$doc.scrollTop();a.focus();window.scrollTo(c,d)}else a.focus()},getSelectedItems:function(){return this.$selectedItems},destroy:function(){this.$container.removeData("select");this.removeAllItems();this.base()},onMouseDown:function(a){if(a.which==Garnish.PRIMARY_CLICK&&(!this.settings.filter||c(a.target).is(this.settings.filter))){this.mousedownX=a.pageX;this.mousedownY=a.pageY;var b=c(c.data(a.currentTarget,"select-item"));null!==this.first&&a.shiftKey?this.selectRange(b,!0):this._actAsCheckbox(a)&&
this.toggleItem(b,!0)}},onMouseUp:function(a){if(a.which==Garnish.PRIMARY_CLICK&&(!this.settings.filter||c(a.target).is(this.settings.filter))){var b=c(c.data(a.currentTarget,"select-item"));!this._actAsCheckbox(a)&&!a.shiftKey&&1>Garnish.getDist(this.mousedownX,this.mousedownY,a.pageX,a.pageY)&&(this.isSelected(b)?(this.clearMouseUpTimeout(),this.mouseUpTimeout=setTimeout(c.proxy(function(){this.deselectOthers(b)},this),300)):(this.deselectAll(),this.selectItem(b,!0,!0)))}},onKeyDown:function(a){if(a.target==
a.currentTarget){var b=Garnish.isCtrlKeyPressed(a);if(this.settings.checkboxMode&&this.$focusable.length)e=c.inArray(this.$focusable[0],this.$items),-1==e&&(e=0);else var e=a.shiftKey?this.last:this.first;switch(a.keyCode){case Garnish.LEFT_KEY:a.preventDefault();var d=null===this.first?Garnish.ltr?this.getLastItem():this.getFirstItem():b?this.getFurthestItemToTheLeft(e):this.getItemToTheLeft(e);break;case Garnish.RIGHT_KEY:a.preventDefault();d=null===this.first?Garnish.ltr?this.getFirstItem():this.getLastItem():
b?this.getFurthestItemToTheRight(e):this.getItemToTheRight(e);break;case Garnish.UP_KEY:a.preventDefault();null===this.first?(this.$focusable&&(d=this.$focusable.prev()),this.$focusable&&d.length||(d=this.getLastItem())):(d=b?this.getFurthestItemAbove(e):this.getItemAbove(e))||(d=this.getFirstItem());break;case Garnish.DOWN_KEY:a.preventDefault();null===this.first?(this.$focusable&&(d=this.$focusable.next()),this.$focusable&&d.length||(d=this.getFirstItem())):(d=b?this.getFurthestItemBelow(e):this.getItemBelow(e))||
(d=this.getLastItem());break;case Garnish.SPACE_KEY:b||(a.preventDefault(),this.isSelected(this.$focusable)?this._canDeselect(this.$focusable)&&this.deselectItem(this.$focusable):this.selectItem(this.$focusable,!0));break;case Garnish.A_KEY:b&&(a.preventDefault(),this.selectAll())}d&&d.length&&(this.settings.checkboxMode?(this.setFocusableItem(d),d.focus()):null!==this.first&&a.shiftKey?this.selectRange(d):(this.deselectAll(),this.selectItem(d,!0)))}},onSelectionChange:function(){this.callbackFrame&&
(Garnish.cancelAnimationFrame(this.callbackFrame),this.callbackFrame=null);this.callbackFrame=Garnish.requestAnimationFrame(c.proxy(function(){this.callbackFrame=null;this.trigger("selectionChange");this.settings.onSelectionChange()},this))},_actAsCheckbox:function(a){return Garnish.isCtrlKeyPressed(a)?!this.settings.checkboxMode:this.settings.checkboxMode},_canDeselect:function(a){return this.settings.allowEmpty||this.totalSelected>a.length},_selectItems:function(a){a.addClass(this.settings.selectedClass);
...
onSelectionChange:c.noop},closestItemAxisProps:{x:{midpointOffset:"top",midpointSizeFunc:"outerHeight",rowOffset:"left"},y:{midpointOffset:"left",midpointSizeFunc:"outerWidth",rowOffset:"top"}},closestItemDirectionProps:{"<":{step:-1,isNextRow:function(a,b){return a<b},isWrongDirection:function(a,b){return a>b}},">":{step:1,isNextRow:function(a,b){return a>b},isWrongDirection:function(a,b){return a<b}}}});Garnish.SelectMenu=Garnish.Menu.extend({init:function(a,b,e,d){"function"==typeof e&&(d=e,e=
{});e=c.extend({},Garnish.SelectMenu.defaults,e);this.base(a,b,e,d);this.selected=-1},build:function(){this.base();-1!=this.selected&&this._addSelectedOptionClass(this.selected)},select:function(a){a!=this.selected&&(this.dom.ul&&(-1!=this.selected&&(this.dom.options[this.selected].className=""),this._addSelectedOptionClass(a)),this.selected=a,this.setBtnText(c(this.options[a].label).text()),this.base(a))},_addSelectedOptionClass:function(a){this.dom.options[a].className="sel"},setBtnText:function(a){this.dom.$btnLabel.text(a)}},
