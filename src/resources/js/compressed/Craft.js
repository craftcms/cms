...
// onAddRow callback
this.settings.onAddRow(b)},createRow:function(a,b,c,d){return Craft.EditableTable.createRow(a,b,c,d)}},{textualColTypes:["singleline","multiline","number"],defaults:{rowIdPrefix:"",onAddRow:a.noop,onDeleteRow:a.noop},createRow:function(b,c,d,e){var f=a("<tr/>",{"data-id":b});for(var g in c)if(c.hasOwnProperty(g)){var h,i=c[g],j="undefined"!=typeof e[g]?e[g]:"";if("heading"==i.type)h=a("<th/>",{scope:"row",class:i.class,html:j});else{var k=d+"["+b+"]["+g+"]",l=Craft.inArray(i.type,Craft.EditableTable.textualColTypes);switch(h=a("<td/>",{class:i.class,width:i.width}),l&&h.addClass("textual"),i.code&&h.addClass("code"),i.type){case"select":Craft.ui.createSelect({name:k,options:i.options,value:j,class:"small"}).appendTo(h);break;case"checkbox":Craft.ui.createCheckbox({name:k,value:i.value||"1",checked:!!j}).appendTo(h);break;case"lightswitch":Craft.ui.createLightswitch({name:k,value:j}).appendTo(h);break;default:a("<textarea/>",{name:k,rows:1,value:j}).appendTo(h)}}h.appendTo(f)}return a("<td/>",{class:"thin action"}).append(a("<a/>",{class:"move icon",title:Craft.t("app","Reorder")})).appendTo(f),a("<td/>",{class:"thin action"}).append(a("<a/>",{class:"delete icon",title:Craft.t("app","Delete")})).appendTo(f),f}}),/**
