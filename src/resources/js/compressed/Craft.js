...
// Make 'data' optional
"function"==typeof c&&(e=d,d=c,c={}),Craft.csrfTokenValue&&Craft.csrfTokenName&&("string"==typeof c?(c&&(c+="&"),c+=Craft.csrfTokenName+"="+Craft.csrfTokenValue):(c="object"!=typeof c?{}:a.extend({},c),c[Craft.csrfTokenName]=Craft.csrfTokenValue));var f=a.ajax(a.extend({url:Craft.getActionUrl(b),type:"POST",data:c,success:d,error:function(a,b,c){d&&d(null,b,a)},complete:function(a,b){"success"!=b&&("undefined"!=typeof Craft.cp?Craft.cp.displayError():alert(Craft.t("An unknown error occurred.")))}},e));
...
getElementInfo:function(b){var c=a(b);c.hasClass("element")||(c=c.find(".element:first"));var d={id:c.data("id"),locale:c.data("locale"),label:c.data("label"),status:c.data("status"),url:c.data("url"),hasThumb:c.hasClass("hasthumb"),$element:c};return d},/**
	 ...
	 * @param size
	 */
setElementSize:function(b,c){var d=a(b);if("small"!=c&&"large"!=c&&(c="small"),!d.hasClass(c)){var e="small"==c?"large":"small";if(d.addClass(c).removeClass(e),d.hasClass("hasthumb")){var f=d.find("> .elementthumb > img"),g="small"==c?"30":"100";$newImg=a("<img/>",{sizes:g+"px",srcset:f.attr("srcset")||f.attr("data-pfsrcset")}),f.replaceWith($newImg),picturefill({elements:[$newImg[0]]})}}},/**
...
// Properties
// =========================================================================
initialized:!1,elementType:null,instanceState:null,sourceStates:null,sourceStatesStorageKey:null,searchTimeout:null,sourceSelect:null,$container:null,$main:null,$mainSpinner:null,isIndexBusy:!1,$sidebar:null,showingSidebar:null,sourceKey:null,sourceViewModes:null,$source:null,$customizeSourcesBtn:null,customizeSourcesModal:null,$toolbar:null,$toolbarTableRow:null,toolbarOffset:null,$search:null,searching:!1,searchText:null,$clearSearchBtn:null,$statusMenuBtn:null,statusMenu:null,status:null,$localeMenuBtn:null,localeMenu:null,locale:null,$sortMenuBtn:null,sortMenu:null,$sortAttributesList:null,$sortDirectionsList:null,$scoreSortAttribute:null,$structureSortAttribute:null,$elements:null,$viewModeBtnTd:null,$viewModeBtnContainer:null,viewModeBtns:null,viewMode:null,view:null,_autoSelectElements:null,actions:null,actionsHeadHtml:null,actionsFootHtml:null,$selectAllContainer:null,$selectAllCheckbox:null,showingActionTriggers:!1,_$triggers:null,
// Public methods
// =========================================================================
/**
	 * Constructor
...
// ---------------------------------------------------------------------
this.$statusMenuBtn.length&&(this.statusMenu=this.$statusMenuBtn.menubtn().data("menubtn").menu,this.statusMenu.on("optionselect",a.proxy(this,"_handleStatusChange"))),this.$localeMenuBtn.length){this.localeMenu=this.$localeMenuBtn.menubtn().data("menubtn").menu;
// Figure out the initial locale
var f=this.localeMenu.$options.filter(".sel:first");if(f.length||(f=this.localeMenu.$options.first()),f.length?this.locale=f.data("locale"):
// No locale options -- they must not have any locale permissions
this.settings.criteria={id:"0"},this.localeMenu.on("optionselect",a.proxy(this,"_handleLocaleChange")),this.locale){
...
h.trigger("click")}}}else this.settings.criteria&&this.settings.criteria.locale&&(this.locale=this.settings.criteria.locale);
...
var d=this.view.getSelectedElementIds(),e=d.length;this.view.getEnabledElements.length;if(0!=e){for(var f,g=0;g<this.actions.length;g++)if(this.actions[g].handle==b){f=this.actions[g];break}if(f&&(!f.confirm||confirm(f.confirm))){
// Get ready to submit
...
this.$selectAllContainer=a('<td class="selectallcontainer thin"/>'),this.$selectAllBtn=a('<div class="btn"/>').appendTo(this.$selectAllContainer),this.$selectAllCheckbox=a('<div class="checkbox"/>').appendTo(this.$selectAllBtn),this.addListener(this.$selectAllBtn,"click",function(){0==this.view.getSelectedElements().length?this.view.selectAllElements():this.view.deselectAllElements()})),
...
"index"==this.settings.context&&this.addListener(this.$elementContainer,"dblclick",function(b){var c=a(b.target);if("A"!=c.prop("nodeName")){var d;if(c.hasClass("element"))d=c;else if(d=c.closest(".element"),!d.length)return;Garnish.hasAttr(d,"data-editable")&&new Craft.ElementEditor(d)}}),
...
// Build the modal
var d=a('<div class="modal elementselectormodal"></div>').appendTo(Garnish.$bod),e=a('<div class="body"><div class="spinner big"></div></div>').appendTo(d),f=a('<div class="footer"/>').appendTo(d);this.base(d,this.settings),this.$footerSpinner=a('<div class="spinner hidden"/>').appendTo(f),this.$primaryButtons=a('<div class="buttons right"/>').appendTo(f),this.$secondaryButtons=a('<div class="buttons left secondary-buttons"/>').appendTo(f),this.$cancelBtn=a('<div class="btn">'+Craft.t("Cancel")+"</div>").appendTo(this.$primaryButtons),this.$selectBtn=a('<div class="btn disabled submit">'+Craft.t("Select")+"</div>").appendTo(this.$primaryButtons),this.$body=e,this.addListener(this.$cancelBtn,"activate","cancel"),this.addListener(this.$selectBtn,"activate","selectElements")},onFadeIn:function(){this.elementIndex?
// Auto-focus the Search box
...
this.addListener(this.elementIndex.$elements,"dblclick","selectElements"))},this))}},{defaults:{resizable:!0,storageKey:null,sources:null,criteria:null,multiSelect:!1,disabledElementIds:[],disableElementsOnSelect:!1,hideOnSelect:!0,onCancel:a.noop,onSelect:a.noop}}),/**
 * Input Generator
 */
...
var e={ids:JSON.stringify(b)};Craft.postActionRequest(this.settings.reorderAction,e,a.proxy(function(a,c){"success"==c&&(a.success?(this.onReorderItems(b),Craft.cp.displayNotice(Craft.t(this.settings.reorderSuccessMessage))):Craft.cp.displayError(Craft.t(this.settings.reorderFailMessage)))},this))},handleDeleteBtnClick:function(b){if(!(this.settings.minItems&&this.totalItems<=this.settings.minItems)){var c=a(b.target).closest("tr");this.confirmDeleteItem(c)&&this.deleteItem(c)}},confirmDeleteItem:function(a){var b=this.getItemName(a);return confirm(Craft.t(this.settings.confirmDeleteMessage,{name:b}))},deleteItem:function(b){var c={id:this.getItemId(b)};Craft.postActionRequest(this.settings.deleteAction,c,a.proxy(function(a,c){"success"==c&&this.handleDeleteItemResponse(a,b)},this))},handleDeleteItemResponse:function(a,b){var c=this.getItemId(b),d=this.getItemName(b);a.success?(this.sorter&&this.sorter.removeItems(b),b.remove(),this.totalItems--,this.updateUI(),this.onDeleteItem(c),Craft.cp.displayNotice(Craft.t(this.settings.deleteSuccessMessage,{name:d}))):Craft.cp.displayError(Craft.t(this.settings.deleteFailMessage,{name:d}))},onReorderItems:function(a){this.settings.onReorderItems(a)},onDeleteItem:function(a){this.settings.onDeleteItem(a)},getItemId:function(a){return a.attr(this.settings.idAttribute)},getItemName:function(a){return a.attr(this.settings.nameAttribute)},updateUI:function(){
...
this._uploadedFileIds.push(d.fileId),d.prompt){var g={message:this._fileConflictTemplate.message,choices:this._fileConflictTemplate.choices};g.message=Craft.t(g.message,{file:d.filename}),d.prompt=g,this.promptHandler.addPrompt(d)}}else d.error?alert(Craft.t("Upload failed for {filename}. The error message was: “{error}”",{filename:fileName,error:d.error})):alert(Craft.t("Upload failed for {filename}.",{filename:e})),f=!1;
...
d.call(this,b,c),this.createSelectTransformButton()},this))}this.base(b,c),c.canSelectImageTransforms&&this.createSelectTransformButton()},fetchTransformInfo:function(b){Craft.postActionRequest("assets/get-transform-info",a.proxy(function(a,c){"success"==c&&a instanceof Array?Craft.AssetSelectorModal.transforms=a:Craft.AssetSelectorModal.transforms=[],b()},this))},createSelectTransformButton:function(){if(Craft.AssetSelectorModal.transforms.length){var b=a('<div class="btngroup"/>').appendTo(this.$primaryButtons);this.$selectBtn.appendTo(b),this.$selectTransformBtn=a('<div class="btn menubtn disabled">'+Craft.t("Select transform")+"</div>").appendTo(b);for(var c=a('<div class="menu" data-align="right"></div>').insertAfter(this.$selectTransformBtn),d=a("<ul></ul>").appendTo(c),e=0;e<Craft.AssetSelectorModal.transforms.length;e++)a('<li><a data-transform="'+Craft.AssetSelectorModal.transforms[e].handle+'">'+Craft.AssetSelectorModal.transforms[e].name+"</a></li>").appendTo(d);var f=new Garnish.MenuBtn(this.$selectTransformBtn,{onOptionSelect:a.proxy(this,"onSelectTransform")});f.disable(),this.$selectTransformBtn.data("menuButton",f)}},onSelectionChange:function(b){var c=this.elementIndex.getSelectedElements(),d=!1,e=null;if(c.length&&this.settings.canSelectImageTransforms&&Craft.AssetSelectorModal.transforms.length){d=!0;for(var f=0;f<c.length;f++)if(!a(".element.hasthumb:first",c[f]).length){d=!1;break}}else d=!1;e=this.$selectTransformBtn?this.$selectTransformBtn.data("menuButton"):null,d?(e&&e.enable(),this.$selectTransformBtn.removeClass("disabled")):this.$selectTransformBtn&&(e&&e.disable(),this.$selectTransformBtn.addClass("disabled")),this.base()},onSelectTransform:function(b){var c=a(b).data("transform");this.selectImagesWithTransform(c)},selectImagesWithTransform:function(b){
...
if(c){var f=this._getGroupTriggerHref(c),g="index"==this.settings.context?Craft.t("New category"):Craft.t("New {group} category",{group:c.name});this.$newCategoryBtn=a('<a class="btn submit add icon" '+f+">"+g+"</a>").appendTo(this.$newCategoryBtnGroup),"index"!=this.settings.context&&this.addListener(this.$newCategoryBtn,"click",function(a){this._openCreateCategoryModal(a.currentTarget.getAttribute("data-id"))}),this.editableGroups.length>1&&(e=a('<div class="btn submit menubtn"></div>').appendTo(this.$newCategoryBtnGroup))}else this.$newCategoryBtn=e=a('<div class="btn submit add icon menubtn">'+Craft.t("New category")+"</div>").appendTo(this.$newCategoryBtnGroup);if(e){for(var h='<div class="menu"><ul>',d=0;d<this.editableGroups.length;d++){var i=this.editableGroups[d];if("index"==this.settings.context||i!=c){var f=this._getGroupTriggerHref(i),g="index"==this.settings.context?i.name:Craft.t("New {group} category",{group:i.name});h+="<li><a "+f+'">'+g+"</a></li>"}}h+="</ul></div>";var j=(a(h).appendTo(this.$newCategoryBtnGroup),new Garnish.MenuBtn(e));"index"!=this.settings.context&&j.on("optionSelect",a.proxy(function(a){this._openCreateCategoryModal(a.option.getAttribute("data-id"))},this))}this.addButton(this.$newCategoryBtnGroup)}
// Update the URL if we're on the Categories index
// ---------------------------------------------------------------------
if("index"==this.settings.context&&"undefined"!=typeof history){var k="categories";b&&(k+="/"+b),history.replaceState({},"",Craft.getUrl(k))}this.base()},_getGroupTriggerHref:function(a){return"index"==this.settings.context?'href="'+Craft.getUrl("categories/"+a.handle+"/new")+'"':'data-id="'+a.id+'"'},_openCreateCategoryModal:function(b){if(!this.$newCategoryBtn.hasClass("loading")){for(var c,d=0;d<this.editableGroups.length;d++)if(this.editableGroups[d].id==b){c=this.editableGroups[d];break}if(c){this.$newCategoryBtn.addClass("inactive");var e=this.$newCategoryBtn.text();this.$newCategoryBtn.text(Craft.t("New {group} category",{group:c.name})),new Craft.ElementEditor({hudTrigger:this.$newCategoryBtnGroup,elementType:"Category",locale:this.locale,attributes:{groupId:b},onBeginLoading:a.proxy(function(){this.$newCategoryBtn.addClass("loading")},this),onEndLoading:a.proxy(function(){this.$newCategoryBtn.removeClass("loading")},this),onHideHud:a.proxy(function(){this.$newCategoryBtn.removeClass("inactive").text(e)},this),onSaveElement:a.proxy(function(a){
...
// sidebar
this.addListener(a("ul",this.$sidebar),"resize","updateResponsiveSidebar"),this.$sidebarLinks=a("nav a",this.$sidebar),this.addListener(this.$sidebarLinks,"click","selectSidebarItem"),this.addListener(this.$container,"scroll","updateFixedNotifications"),this.updateFixedNotifications(),Garnish.$doc.ready(a.proxy(function(){
...
this.$globalSidebarTopbar.children("a.updates").remove(),b.total){var c;c=1==b.total?Craft.t("1 update available"):Craft.t("{num} updates available",{num:b.total}),
// Topbar badge
 ...
 * Element editor
 */
Craft.ElementEditor=Garnish.Base.extend({$element:null,elementId:null,locale:null,$form:null,$fieldsContainer:null,$cancelBtn:null,$saveBtn:null,$spinner:null,$localeSelect:null,$localeSpinner:null,hud:null,init:function(b,c){
// Param mapping
"undefined"==typeof c&&a.isPlainObject(b)&&(
// (settings)
c=b,b=null),this.$element=b,this.setSettings(c,Craft.ElementEditor.defaults),this.loadHud()},setElementAttribute:function(a,b){this.settings.attributes||(this.settings.attributes={}),null===b?delete this.settings.attributes[a]:this.settings.attributes[a]=b},getBaseData:function(){var a={};return this.settings.locale?a.locale=this.settings.locale:this.$element&&this.$element.data("locale")&&(a.locale=this.$element.data("locale")),this.settings.elementId?a.elementId=this.settings.elementId:this.$element&&this.$element.data("id")&&(a.elementId=this.$element.data("id")),this.settings.elementType&&(a.elementType=this.settings.elementType),this.settings.attributes&&(a.attributes=this.settings.attributes),a},loadHud:function(){this.onBeginLoading();var b=this.getBaseData();b.includeLocales=this.settings.showLocaleSwitcher,Craft.postActionRequest("elements/get-editor-html",b,a.proxy(this,"showHud"))},showHud:function(b,c){if(this.onEndLoading(),"success"==c){var d=a();if(b.locales){var e=a('<div class="header"/>'),f=a('<div class="select"/>').appendTo(e);this.$localeSelect=a("<select/>").appendTo(f),this.$localeSpinner=a('<div class="spinner hidden"/>').appendTo(e);for(var g=0;g<b.locales.length;g++){var h=b.locales[g];a('<option value="'+h.id+'"'+(h.id==b.locale?' selected="selected"':"")+">"+h.name+"</option>").appendTo(this.$localeSelect)}this.addListener(this.$localeSelect,"change","switchLocale"),d=d.add(e)}this.$form=a("<div/>"),this.$fieldsContainer=a('<div class="fields"/>').appendTo(this.$form),this.updateForm(b);var i=a('<div class="footer"/>').appendTo(this.$form),j=a('<div class="buttons right"/>').appendTo(i);if(this.$cancelBtn=a('<div class="btn">'+Craft.t("Cancel")+"</div>").appendTo(j),this.$saveBtn=a('<input class="btn submit" type="submit" value="'+Craft.t("Save")+'"/>').appendTo(j),this.$spinner=a('<div class="spinner hidden"/>').appendTo(j),d=d.add(this.$form),this.hud)this.hud.updateBody(d),this.hud.updateSizeAndPosition();else{var k=this.settings.hudTrigger||this.$element;this.hud=new Garnish.HUD(k,d,{bodyClass:"body elementeditor",closeOtherHUDs:!1,onShow:a.proxy(this,"onShowHud"),onHide:a.proxy(this,"onHideHud"),onSubmit:a.proxy(this,"saveElement")}),this.hud.$hud.data("elementEditor",this),this.hud.on("hide",a.proxy(function(){delete this.hud},this))}
// Focus on the first text input
d.find(".text:first").focus(),this.addListener(this.$cancelBtn,"click",function(){this.hud.hide()})}},switchLocale:function(){var b=this.$localeSelect.val();if(b!=this.locale){this.$localeSpinner.removeClass("hidden");var c=this.getBaseData();c.locale=b,Craft.postActionRequest("elements/get-editor-html",c,a.proxy(function(a,b){this.$localeSpinner.addClass("hidden"),"success"==b?this.updateForm(a):this.$localeSelect.val(this.locale)},this))}},updateForm:function(b){this.locale=b.locale,this.$fieldsContainer.html(b.html);for(var c=this.$fieldsContainer.find("> .meta > .field > .heading > .instructions"),d=0;d<c.length;d++)c.eq(d).replaceWith(a("<span/>",{class:"info",html:c.eq(d).children().html()})).infoicon();Garnish.requestAnimationFrame(a.proxy(function(){Craft.appendHeadHtml(b.headHtml),Craft.appendFootHtml(b.footHtml),Craft.initUiElements(this.$fieldsContainer)},this))},saveElement:function(){this.$spinner.removeClass("hidden");var b=a.param(this.getBaseData())+"&"+this.hud.$body.serialize();Craft.postActionRequest("elements/save-element",b,a.proxy(function(a,b){if(this.$spinner.addClass("hidden"),"success"==b)if("success"==b&&a.success){if(this.$element&&this.locale==this.$element.data("locale")){
// Update the label
...
// Otherwise only show a menu button
if(c){var f=this._getSectionTriggerHref(c),g="index"==this.settings.context?Craft.t("New entry"):Craft.t("New {section} entry",{section:c.name});this.$newEntryBtn=a('<a class="btn submit add icon" '+f+">"+g+"</a>").appendTo(this.$newEntryBtnGroup),"index"!=this.settings.context&&this.addListener(this.$newEntryBtn,"click",function(a){this._openCreateEntryModal(a.currentTarget.getAttribute("data-id"))}),this.publishableSections.length>1&&(e=a('<div class="btn submit menubtn"></div>').appendTo(this.$newEntryBtnGroup))}else this.$newEntryBtn=e=a('<div class="btn submit add icon menubtn">'+Craft.t("New entry")+"</div>").appendTo(this.$newEntryBtnGroup);if(e){for(var h='<div class="menu"><ul>',d=0;d<this.publishableSections.length;d++){var i=this.publishableSections[d];if("index"==this.settings.context||i!=c){var f=this._getSectionTriggerHref(i),g="index"==this.settings.context?i.name:Craft.t("New {section} entry",{section:i.name});h+="<li><a "+f+'">'+g+"</a></li>"}}h+="</ul></div>";var j=(a(h).appendTo(this.$newEntryBtnGroup),new Garnish.MenuBtn(e));"index"!=this.settings.context&&j.on("optionSelect",a.proxy(function(a){this._openCreateEntryModal(a.option.getAttribute("data-id"))},this))}this.addButton(this.$newEntryBtnGroup)}
// Update the URL if we're on the Entries index
// ---------------------------------------------------------------------
if("index"==this.settings.context&&"undefined"!=typeof history){var k="entries";b&&(k+="/"+b),history.replaceState({},"",Craft.getUrl(k))}this.base()},_getSectionTriggerHref:function(a){return"index"==this.settings.context?'href="'+Craft.getUrl("entries/"+a.handle+"/new")+'"':'data-id="'+a.id+'"'},_openCreateEntryModal:function(b){if(!this.$newEntryBtn.hasClass("loading")){for(var c,d=0;d<this.publishableSections.length;d++)if(this.publishableSections[d].id==b){c=this.publishableSections[d];break}if(c){this.$newEntryBtn.addClass("inactive");var e=this.$newEntryBtn.text();this.$newEntryBtn.text(Craft.t("New {section} entry",{section:c.name})),new Craft.ElementEditor({hudTrigger:this.$newEntryBtnGroup,elementType:"Entry",locale:this.locale,attributes:{sectionId:b},onBeginLoading:a.proxy(function(){this.$newEntryBtn.addClass("loading")},this),onEndLoading:a.proxy(function(){this.$newEntryBtn.removeClass("loading")},this),onHideHud:a.proxy(function(){this.$newEntryBtn.removeClass("inactive").text(e)},this),onSaveElement:a.proxy(function(a){
 ...
 * Handle Generator
 */
Craft.EntryUrlFormatGenerator=Craft.BaseInputGenerator.extend({generateTargetValue:function(a){
// Remove HTML tags
a=a.replace("/<(.*?)>/g",""),
// Make it lowercase
a=a.toLowerCase(),
...
Craft.cp.$collapsibleTables=Craft.cp.$collapsibleTables.not(this.$table)),this.base()},_collapseElement:function(a,b){if(!b&&!a.hasClass("expanded"))return!1;a.removeClass("expanded");for(
...
this.couponPrice=null,this.updateCheckoutUi()},onHide:function(){this.initializedCheckoutForm&&(this.$businessCountryInput[0].selectize.blur(),this.$businessStateInput[0].selectize.blur()),this.clearCheckoutFormInABit(),this.base()},onBuyBtnClick:function(b){var c=a(b.currentTarget);switch(this.edition=c.data("edition"),this.couponPrice=null,this.formattedCouponPrice=null,this.edition){case 1:this.$checkoutLogo.attr("class","logo craftclient").text("Craft Client");break;case 2:this.$checkoutLogo.attr("class","logo craftpro").text("Craft Pro")}this.updateCheckoutUi(),this.clearCheckoutFormTimeout&&clearTimeout(this.clearCheckoutFormTimeout);
// Slide it in
...
// Static Properties
// =============================================================================
{defaults:{dropZone:null,pasteZone:null,fileInput:null,sequentialUploads:!0,maxFileSize:Craft.maxUploadSize,allowedKinds:null,events:{},canAddMoreFiles:null}})}(jQuery);
