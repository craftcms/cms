(function($) {
	...
	Craft.FindAndReplaceUtility = Garnish.Base.extend(
	{
		...
		postActionRequest: function(params)
		{
			...
			Craft.postActionRequest('utilities/find-and-replace-perform-action', data, $.proxy(this, 'onActionResponse'),
				{
					complete: $.noop
				});
		},
		...
		onActionResponse: function(response, textStatus)
		{
			...
			while (this.loadingActions < Craft.FindAndReplaceUtility.maxConcurrentActions && this.currentBatchQueue.length) {
				this.loadNextAction();
			}
