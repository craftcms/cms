...
class App
{
    ...
    public static function envConfig(object|string $class, ?string $envPrefix = null): array
    {
        ...
        $properties = $model
            ...
            : Collection::make($reflection->getProperties(ReflectionProperty::IS_PUBLIC))
                ->filter(fn(ReflectionProperty $prop) => !$prop->isStatic())
                ->map(fn(ReflectionProperty $prop) => $prop->getName());

        return $properties
            ->mapWithKeys(function(string $propName) use ($envPrefix, $reflection) {
                $envName = static::envNameForProperty($reflection, $propName)
                    ?? strtoupper(StringHelper::toSnakeCase($propName));
                $envValue = static::env($envPrefix . $envName);
    ...
    /**
     * Returns the environment variable name defined by a property’s `EnvName` attribute, if it has one.
     *
     ...
     * @return string|null
     */
    private static function envNameForProperty(ReflectionClass $reflection, string $propName): ?string
    {
        ...
        foreach ($reflection->getProperty($propName)->getAttributes(EnvName::class) as $attribute) {
            /** @var EnvName $envName */
