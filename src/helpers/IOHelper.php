...
class IOHelper
{
	 ...
	 * @param bool   $caseInsensitive Whether to perform a case insensitive
	 *                                check or not.
	 * @param bool   $suppressErrors  Whether to suppress any PHP Notices/Warnings/Errors
	...
	/**
	 * If the file exists on the file system will return a new File instance,
	 * otherwise, false.
	 *
	...
	/**
	 * If the folder exists on the file system, will return a new Folder instance,
	 * otherwise, false.
	 *
	...
	/**
	 * If the path exists on the file system, will return the paths of any folders
	...
	/**
	 * If the path exists on the file system, will return the paths of any files
	 * that are contained within it.
	 *
	 * @param string $path           The folder path to check
	 * @param bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	...
	/**
	 * Tests file and folder write-ability by attempting to create a temp file on
	 * the filesystem. PHP's is_writable has problems (especially on Windows).
	 * {@see https://bugs.php.net/bug.php?id=27609} and {@see https://bugs.php.net/bug.php?id=30931}.
	 *
	 * @param string $path           The path to test.
	 * @param bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                               (usually permissions related).
	 ...
	 *
	 * @param string $path             The path to test.
	 * @param bool   $includeExtension Whether to include the extension in the
	 *                                 file name.
	 * @param bool   $suppressErrors   Whether to suppress any PHP Notices/Warnings/Errors
	 *                                 (usually permissions related).
	 *
	 ...
	 *
	 * @param string $path           The path to test.
	 * @param bool   $fullPath       Whether to include the full path in the return
	 *                               results or the top level folder only.
	 * @param bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                               (usually permissions related).
	 *
	...
	public static function getFolderName($path, $fullPath = true, $suppressErrors = false)
	{
		...
		if ($fullPath)
		{
			...
			// normalizePathSeparators() only enforces the trailing slash for
			// known directories so let's be sure that it'll be there.
			return rtrim($folder, '/').'/';
		}
		else
		{
	...
	/**
	 * Returns the file extension for the given path.  If there is not one, then
	 * $default is returned instead.
	 *
	 * @param string      $path           The path to test.
	 * @param null|string $default        If the file has no extension, this one
	...
	/**
	 * If the path points to a real file, we call {@link CFileHelper::getMimeType},
	 * otherwise {@link CFileHelper::getMimeTypeByExtension}
	 *
	 * @param string $path The path to test.
	 *
	...
	/**
	 * Returns the last modified time for the given path in DateTime format or
	 * false if the file or folder does not exist.
	 *
	 * @param string $path           The path to test.
	 * @param bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                               (usually permissions related).
	 *
	 * @return int|bool The last modified timestamp or false if the file or folder
	 *                  does not exist.
	 */
	...
	/**
	 * Returns the file size in bytes for the given path or false if the file does
	 * not exist.
	 *
	 * @param string $path           The path to test.
	 * @param bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	...
	/**
	 * Returns the folder size in bytes for the given path or false if the folder
	 * does not exist.
	 *
	 * @param string $path           The path to test.
	 * @param bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	...
	/**
	 * Will take a given path and normalize it to use single forward slashes for
	 * path separators.  If it is a folder, it will append a trailing forward
	...
	public static function normalizePathSeparators($path)
	{
		...
		// Check if the path is just a slash.  If the server has openbase_dir
		// restrictions in place calling is_dir on it will complain.
		if ($path !== '/')
		{
	...
	/**
	 * Will take a path, make sure the file exists and if the size of the file is
	 * 0 bytes, return true.  Otherwise false.
	 *
	 * @param string $path         The path to test.
	 * @param bool $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                             (usually permissions related).
	...
	/**
	 * Will take a path, make sure the folder exists and if the size of the
	 * folder is 0 bytes, return true.  Otherwise false.
	 *
	 * @param string $path           The path to test.
	 * @param bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                               (usually permissions related).
	...
	/**
	 * Returns owner of current filesystem object (UNIX systems). Returned value
	 ...
	 *                                (usually permissions related).
	 *
	 * @return mixed Owner name, or ID if $getName set to 'false' or false if the
	 ...
	 *
	 * @param string  $path           The path to check.
	 * @param boolean $getName        Defaults to 'true', meaning that group
	 *                                name instead of ID should be returned.
	 * @param bool    $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                                (usually permissions related).
	 *
	 * @return mixed Group name, or ID if $getName set to 'false' or false if the
	 *               file or folder does not exist.
	 */
	 ...
	 *                false if the file or folder doesn't exist
	 */
	...
	/**
	 * Returns the contents of a folder as an array of file and folder paths, or
	 * false if the folder does not exist or is not readable.
	 *
	 * @param  string $path               The path to test.
	 ...
	 * @param  bool   $includeHiddenFiles Whether to include hidden files (that start
	 *                                    with a .) in the results.
	 * @param  bool   $suppressErrors     Whether to suppress any PHP Notices/Warnings/Errors
	 *                                    (usually permissions related).
	 *
	 * @return array|bool An array of file and folder paths, or false if the folder
	...
	/**
	 * Will return the contents of the file as a string or an array if it exists
	 * and is readable, otherwise false.
	 *
	 * @param  string $path           The path of the file.
	 * @param  bool   $array          Whether to return the contents of the file
	 *                                as an array or not.
	 * @param  bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                                (usually permissions related).
	 *
	 * @return bool|string|array The contents of the file as a string, an array,
	...
	/**
	 * Will create a file on the file system at the given path and return a {@link File}
	 * object or false if we don't have write permissions.
	 *
	 * @param string    $path           The path of the file to create.
	 * @param bool      $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                                  (usually permissions related).
	 *
	 * @return File|bool The newly created file as a {@link File} object or
	 *                   false if we don't have write permissions.
	 */
	...
	/**
	 * Will create a folder on the file system at the given path and return a {@link Folder}
	 * object or false if we don't have write permissions.
	 *
	 * @param string $path           The path of the file to create.
	 ...
	 *                               (usually permissions related).
	 *
	 * @return Folder|bool The newly created folder as a {@link Folder} object
	 *                     or false if we don't have write permissions.
	 */
	 ...
	 * @param bool      $autoCreate     Whether or not to auto-create the file if it
	 *                                  does not exist.
	 * @param bool      $append         If true, will append the data to the contents
	 *                                  of the file, otherwise it will overwrite
	 *                                  the contents.
	 * @param bool|null $noFileLock     Whether to use file locking when writing
	 *                                  to the file.
	 * @param bool      $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                                  (usually permissions related).
	 *
	...
	public static function writeToFile($path, $contents, $autoCreate = true, $append = false, $noFileLock = null, $suppressErrors = false)
	{
		...
		if (static::isWritable($path, $suppressErrors))
		{
			...
			{
				// We haven't cached file lock information yet and this is not a
				// noFileLock request.
	 ...
	 * @param bool   $recursive      If the path is a folder, whether to change
	 *                               the owner of all of the folder's children.
	 * @param bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                               (usually permissions related).
	 *
	 ...
	 * @param bool   $recursive      If the path is a directory, whether to
	 *                               recursively change the group of the child files and folders.
	 * @param bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                               (usually permissions related).
	 *
	 * @return bool 'true' if successful, 'false' if not, or the given path does
	 ...
	 *                               (usually permissions related).
	 *
	 * @return bool 'true' if the copy was successful, 'false' if it was not, the
	 *              source file is not readable or does not exist.
	 */
	 ...
	 * @param bool   $validate       Whether to compare the size of the folders
	 *                               after the copy is complete.
	 * @param bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                               (usually permissions related).
	 *
	 * @return bool 'true' if the copy was successful, 'false' if it was not, or
	 *              $validate is true and the size of the folders do not match
	 *              after the copy.
	 */
	 ...
	 *                               (usually permissions related).
	 *
	 * @return bool 'true' if successful, 'false' if not or the source file or
	...
	public static function rename($path, $newName, $suppressErrors = false)
	{
		...
		if (static::fileExists($path, $suppressErrors) || static::folderExists($path, $suppressErrors))
		{
			// If we're renaming a file and there is no extension on the new name,
			// default to the old extension
	 ...
	 *                               (usually permissions related).
	 *
	 * @return bool 'true' if the file was successfully cleared, 'false' if it
	 *              wasn't, if the file is not writable or the file does not exist.
	 */
	 ...
	 *                               (usually permissions related).
	 *
	 * @return bool 'true' if is successfully purges the folder, 'false' if the
	 *              folder does not exist.
	 */
	 ...
	 * @param bool   $suppressErrors Whether to suppress any errors (usually
	 *                               permissions related) when deleting the file.
	 *
	 * @return bool 'true' if successful, 'false' if it cannot be deleted, it
	 ...
	 * @param bool   $suppressErrors Whether to suppress any errors (usually
	 *                               permissions related) when deleting the folder.
	 *
	 * @return bool 'true' if successful, 'false' if it cannot be deleted, it does
	 *              not exist or it is not writable.
	 */
	...
	/**
	 * Calculates the MD5 hash for a given file path or false if one could not
	 * be calculated or the file does not exist.
	 *
	 * @param string $path           The path of the file to calculate.
	 * @param bool   $suppressErrors Whether to suppress any PHP Notices/Warnings/Errors
	 *                               (usually permissions related).
	 *
	 * @return bool|string The MD5 hash or false if it does not exist, isn't
	 *                     readable or could not be calculated.
	 */
	...
	/**
	 * Applies an array of filter rules to the string representing the file path.
	 * Used internally by {@link dirContents} method.
	 *
	 * @param  string $str    String representing file path to be filtered
	 * @param  array  $filter An array of filter rules, where each rule is a string,
	 *                        supposing that the string starting with '/' is a
	 *                        regular expression. Any other string treated as an
	 *                        extension part of the given filepath (eg. file extension)
	 *
