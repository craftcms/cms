...
- Tab order follows reading order and is predictable.
    - CSS that visually repositions content independent of source order (CSS Grid `grid-template-areas`, the `order` property, flex/grid direction changes per breakpoint) is a common way this silently breaks — visual order can diverge from DOM/tab order even though nothing about focus itself was touched. This isn't automatically wrong: sometimes the diverging order is fine, or unavoidable, for the content involved. But whenever a component's visual arrangement changes per breakpoint via CSS positioning (not by rendering different DOM), manually verify — tab through in a real browser — that focus order still matches the reading order at *each* breakpoint, not just the one that was tested.
...
### Focus on screen changes that aren't real navigations (MUST)

When a component swaps its visible "screen" (a wizard step, a slideout body,
a tab panel) without a real page navigation, nothing resets focus or
announces the change on its own — you must do it explicitly:

- Give the new screen's heading `tabindex="-1"` and move focus to it right
  after the DOM updates (`onMounted` when the screen is its own component
  that gets freshly mounted per change; a `watch` on whatever drives the
  swap, followed by `nextTick()`, when it's conditionally rendered inline
  within the same component instance).
- Prefer focusing the heading over the first form field — it announces
  *what changed* before dropping the user into a control.
- If only one screen/step is ever mounted at a time, its heading is that
  screen's page topic — use `<h1>`, per the "prefer one `h1` for the page
  topic" rule above, not `<h2>`.
- See `resources/js/modules/auth-method-setup/auth-method-setup-slideout.ts`'s
  `showSuccess()` for a local (non-navigation) example, and
  `handleAccessibleRouting()` in `resources/js/bootstrap/cp.ts` +
  `#route-focus-anchor` in `PageScreen.vue` for the equivalent handling real
  Inertia navigations. `resources/js/pages/install/Install.vue` and its
  `StepScreen.vue`/`InstallingScreen.vue` children are a worked example of
  the wizard-step case.
...
- Do not rely on color alone to convey information (error/success/required/selected, links within surrounding text). Provide text and/or icons with accessible names; for a link surrounded by non-link text, add a persistent underline, bold/weight change, or other non-color cue rather than relying on color alone — present by default, removed on hover (not the reverse). See `craft-button`'s `link` variant (`packages/craftcms-ui/src/components/button/button.styles.ts`) and `CpLink`'s `underline` prop (`resources/js/common/components/CpLink.vue`) as existing examples to reuse.
