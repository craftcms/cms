...
it('projects core link type settings under stable local input names', function () {
    ...
    $entryDefinition = $entry->getSettingsForm(false)?->toArray();
    $assetDefinition = $asset->getSettingsForm(false)?->toArray();
    ...
    expect(linkInputNames($entryDefinition))->toBe([
        ...
        'label' => 'All',
        'value' => '*',
    ])->and(linkInputNames(new Url()->getSettingsForm(false)?->toArray()))->toBe([
        'allowRootRelativeUrls',
        'allowAnchors',
        'allowCustomSchemes',
    ])->and(new Email()->getSettingsForm(false))->toBeNull()
        ->and(new Phone()->getSettingsForm(false))->toBeNull()
        ->and(new Sms()->getSettingsForm(false))->toBeNull();
});
