...
it('renders body content', function () {
    $result = $this->manager->renderString('{% cache %}Cached content{% endcache %}');

    expect(trim((string) $result))->toBe('Cached content');
});

it('caches content on second render with global cache', function () {
    $result1 = $this->manager->renderString('{% cache globally using key "test-key" %}Hello{% endcache %}');
    $result2 = $this->manager->renderString('{% cache globally using key "test-key" %}Goodbye{% endcache %}');
...
it('renders dynamic content when cache is bypassed via unless', function () {
    $result1 = $this->manager->renderString('{% cache using key "unless-key" unless true %}First{% endcache %}');
    $result2 = $this->manager->renderString('{% cache using key "unless-key" unless true %}Second{% endcache %}');
