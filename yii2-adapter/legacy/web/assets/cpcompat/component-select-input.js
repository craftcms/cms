/**
 * Backwards-compatibility implementation of the legacy `Craft.ComponentSelectInput`
 * jQuery/Garnish class.
 *
 * Core no longer instantiates this class — every core CP surface renders the
 * self-booting `<craft-component-select>` web component instead. It survives
 * only for the `_includes/forms/componentSelect.twig` `jsClass` escape hatch:
 * third-party plugins that render a component select with an explicit
 * `jsClass` (typically a `Craft.ComponentSelectInput` subclass) still boot the
 * old disclosure-menu markup through this constructor.
 *
 * It used to live in the craftcms-legacy CP bundle
 * (`cp/src/js/ComponentSelectInput.js`). It was relocated here, unchanged, so
 * the core bundle can drop it while plugin subclasses keep a working base
 * class. Unlike cp-compat.js this is the real implementation, not a warn stub,
 * and it is intentionally silent (no deprecation warning) — a straight BC
 * relocation.
 *
 * The class is assigned at top-level eval (NOT on DOMContentLoaded like
 * cp-compat.js): a plugin's `Craft.Foo = Craft.ComponentSelectInput.extend(...)`
 * can run as soon as the plugin's own bundle loads, so the base must already
 * exist by then. The core CP bundle (which defines `Craft`, `Garnish`, and
