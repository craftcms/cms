...
- The `allowAutoUpdates` genral config has been renamed to `allowUpdates`, and now also provides control over plugin installation in the Plugin Store. Support for `minor-only` and `path-only`, the `allowUpdates` config only accepts `true` or `false` values. 
