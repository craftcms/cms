...
* They must implement the [IElementAction](http://buildwithcraft.com/classreference/elementactions/IElementAction) interface.

> {tip} To keep your Element Action class lean and mean, it can inherit from [BaseElementAction](http://buildwithcraft.com/classreference/elementactions/BaseElementAction) instead of implementing the whole IElementAction interface on its own.
...
The choice is up to the Element Action class. If [getTriggerHtml()](http://buildwithcraft.com/classreference/elementactions/IElementAction#getTriggerHtml-detail) returns anything, then whatever it returns will show up the toolbar; otherwise a new option will be added to the menu for you, labeled with whatever [getName()](http://buildwithcraft.com/classreference/etc/components/IComponentType#getName-detail) returns.
...
1. Our [getTriggerHtml()](http://buildwithcraft.com/classreference/elementactions/IElementAction#getTriggerHtml-detail) method fetches all the ingredients from our plugin’s imaginary [service](services.md), passes them to a ‘cocktailRecipes/\_addIngredientsTrigger’ template, and returns the template’s rendered HTML.
...
4. On the back end, when an action request comes in, Craft will instantiate a new CocktailRecipes_AddIngredientElementAction object. Craft knows that it’s expecting an “ingredients” parameter because [defineParams()](http://buildwithcraft.com/classreference/elementactions/BaseElementAction#defineParams-detail) has defined it, so Craft will check the POST data for it.
5. Once any parameters have been assigned, Craft will call the [performAction()](http://buildwithcraft.com/classreference/elementactions/IElementAction#performAction-detail) method, passing an [ElementCriteriaModel](http://buildwithcraft.com/classreference/models/ElementCriteriaModel) to it that represents all of the selected elements.
...
7. The index page is notified of the action’s result, the elements are refreshed, and the message set by [setMessage()](http://buildwithcraft.com/classreference/elementactions/BaseElementAction#setMessage-detail) is displayed to the user.
...
1. Our Element Action doesn’t have a [getTriggerHtml()](http://buildwithcraft.com/classreference/elementactions/IElementAction#getTriggerHtml-detail) method, so instead of getting its own trigger in the toolbar, Craft simply gives it an option in the actions menu, labelled “Remove all ingredients” because that’s what [getName()](http://buildwithcraft.com/classreference/etc/components/IComponentType#getName-detail) returned. It will show up below a horizontal rule in the option menu, because [isDestructive()](http://buildwithcraft.com/classreference/elementactions/IElementAction#isDestructive-detail) returned `true`.
...
5. The index page is notified of the action’s result, the elements are refreshed, and the message set by [setMessage()](http://buildwithcraft.com/classreference/elementactions/BaseElementAction#setMessage-detail) is displayed to the user.
...
To get started, add this to your [getTriggerHtml()](http://buildwithcraft.com/classreference/elementactions/IElementAction#getTriggerHtml-detail) method:
...
Once you’ve created an Element Action class, you need to add it to the appropriate element index page(s).

If your Element Action is intended for your own custom Element Type, you can do that by implementing the [getAvailableActions()](http://buildwithcraft.com/classreference/elementtypes/IElementType#getAvailableActions-detail) method on your Element Type class.
