...
- Added native Forms for `CraftCms\Cms\Field\ContentBlock` and `CraftCms\Cms\Field\Matrix` settings, including structured Field Layout, entry type selection, and site settings via `CraftCms\Cms\Cp\Components\EditableTable` and `<craft-editable-table>`.
