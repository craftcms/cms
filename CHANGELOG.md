...
- Plugins’ `getSettings()` methods must now have a `?Model` return type declaration.
- Plugins’ `install()` methods must now have a `void` return type declaration, and no longer return `null` or `false`.
