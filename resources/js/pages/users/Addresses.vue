    ...
    const createNestedElement = async () => {
        ...
        try {
            ...
            if (!response.cpEditUrl) {
                throw new Error(
                    'The new address came back without an edit URL.'
                );
            }
        ...
        <craft-pane appearance="raised">
            <div ref="cardsContainer" class="grid gap-3">
                ...
                <div v-if="cardsData?.elements.length" class="card-grid">
                    <template
                        v-for="element in cardsData?.elements"
                        :key="element.id"
                    >
                        <craft-card
                            v-bind="
                                attrs(element.cardAttributes, {
                                    exclude: ['class'],
                                })
                            "
                        ...
                        >
                            <div slot="label">
                                <DynamicHtmlRenderer
                                    :html="element.cardLabelHtml"
                                />
                            </div>
                            <div slot="actions">
                                <DynamicHtmlRenderer
                                    :html="element.cardActionsHtml"
                                />
                            </div>
                            <div v-if="element.cardThumbHtml" slot="thumbnail">
                                <DynamicHtmlRenderer
                                    :html="element.cardThumbHtml"
                                />
                            </div>
                            <DynamicHtmlRenderer
                                :html="element.cardContentHtml"
                            />
                        </craft-card>
                    </template>
                </div>
