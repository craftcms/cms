...
export class FieldLayoutDesigner extends Base<FieldLayoutDesignerSettings> {
    ...
    updateFieldSearchResults(): void {
        ...
        for (const group of this.$fieldGroups) {
            if (
                group.querySelector('.fld-element:not(.hidden):not(.filtered)')
            ) {
...
`);
        ...
        void openSlideout(Craft.getCpUrl('settings/fields/edit'), {
            ...
            onSaved: ({data}) => {
                ...
                if (Object(selectorHtml).constructor !== String) {
                    throw new Error(
                        'The saved field response omitted selector HTML.'
                    );
                }
                ...
                if (!$lastGroup) {
                    throw new Error(
                        'The field library requires at least one group.'
                    );
                }
    ...
    cloneLibraryElementForSelection(libraryElement: HTMLElement): HTMLElement {
        ...
        if (!hasAttr($libraryElement, 'data-is-multi-instance')) {
            ...
            const visibleSibling = Array.from(
                ...
                (el) =>
                    el !== $libraryElement &&
                    el.matches('.fld-field:not(.hidden)')
            );
            if (!visibleSibling) {
                $libraryElement
    ...
    static async createSlideout(
        ...
        if (!('payload' in $form) || !('requestData' in $form)) {
            throw new Error(
                'Layout component settings form is not registered.'
            );
        }
