...
class MarkdownField extends LitElement {
    ...
    get value(): string {
        return (
            this.editor?.getValue() ?? this.formValue ?? this.textContent ?? ''
        );
    }
        ...
        const toolbarButtons = toolbarItems(
            ...
            },
            {
                openLinkPopover: (event) =>
                    this.linkPopoverController?.open(event),
    ...
    private statsFormatter: NonNullable<Options['statsFormatter']> = (
        stats
    ) => {
        return `
    ...
    private syncInitialFormValue(inputName: string): void {
        ...
        if (
            this.serializedWithoutInput(serialized, inputName) !== initialValue
        ) {
            return;
        }
