...
function siblingMatch(
    ...
    let sib =
        dir === 'prev' ? el.previousElementSibling : el.nextElementSibling;
    while (sib) {
...
export class Tabs extends Base<TabsSettings> {
    ...
    #handleNavKeydown(ev: any): void {
        ...
        if (
            ...
            const goPrev =
                ev.key ===
                (Craft.orientation === 'ltr' ? 'ArrowLeft' : 'ArrowRight');
            if (goPrev) {
                tab =
                    siblingMatch(
                        current,
    ...
    updateMenuBtn(): void {
        ...
        if (
            Math.floor(this.#tablist.scrollWidth - 48) >
