...
export class EditableTable extends Base<EditableTableSettings> {
    ...
    init(
        ...
        if (tableEl && editableTableData.get(tableEl)) {
            console.warn(
                'Double-instantiating an editable table on an element'
            );
        ...
        for (let i = 0; i < $rows.length; i++) {
            ...
            const rowId = parseInt(
                $row
                    .attr('data-id')
                    .substring(this.settings!.rowIdPrefix.length)
            );
            ...
            this.addListener(
                ...
                (ev: any) => {
                    ...
                    if ($tr.length && !editableTableRowData.get($tr[0])) {
                        const $textarea = $target.hasClass(
    ...
    addRow(focus?: boolean, prepend?: boolean): Row | undefined {
        ...
        if (focus !== false) {
            $tr.find('input:visible,textarea:visible,select:visible')
                .first()
                .focus();
        }
    ...
    static createRow(
        ...
        for (const colId in columns) {
            ...
            const value =
                typeof values[colId] !== 'undefined' ? values[colId] : '';
            let $cell;
            ...
            if (col.type === 'heading') {
                        ...
                        Craft.ui
                            .createSelect({
                                ...
                                value:
                                    value ||
                                    (function () {
                                        const options = col.options as Record<
                                            string,
                                            any
                                        >;
                ...
                if (col.textExpanderTriggers && textExpanderTarget) {
                    ...
                    textExpanderTarget.setAttribute(
                        'aria-label',
                        col.heading ?? colId
                    );
...
export class Row extends Base {
    ...
    init(table: EditableTable, tr: any): void {
        ...
        for (colId in this.table.columns!) {
            if (
                !Object.prototype.hasOwnProperty.call(this.table.columns, colId)
            ) {
                continue;
            }
            ...
            if (Craft.inArray(col.type, EditableTable.textualColTypes)) {
                ...
                if (col.radioMode) {
                    if (
                        typeof this.table.radioCheckboxes[colId] === 'undefined'
                    ) {
            ...
            if (!$(td).hasClass('disabled')) {
                this.addListener(td, 'click', {td}, (ev: any) => {
                    if (ev.target === ev.data.td) {
                        $(ev.data.td)
                            .find('textarea,input,select,.lightswitch')
                            .focus();
                    }
                });
            }
        ...
        if ($actionMenuBtn.length) {
            ...
            this.actionDisclosure.on('show', () => {
                ...
                const $focusableBtn = Garnish.firstFocusableElement(
                    this.$actionMenu
                );
                $focusableBtn.focus();
            });
