    ...
    override onDragStop(): void {
        ...
        if (this._targetLevel != this._draggeeLevel) {
            ...
            for (let i = 0; i < $draggee.length; i++) {
                ...
                const $structureTextAlternative = $row.find(
                ...
                $row.find('> [data-titlecell]:first > div').css(
                    `padding-${Craft.left}`,
                    padding
                );
            ...
            while ($prevRow.length) {
                ...
                if (prevRowLevel < this._targetLevel) {
                    ...
                    if (!$toggle.hasClass('expanded')) {
                        ...
                        // Add a temporary row
                        $spinnerRow =
                            this.tableView._createSpinnerRowAfter($prevRow);

                        // Remove the target item
                        if (this.tableView.elementSelect) {
                            this.tableView.elementSelect.removeItems(
                                $targetItem
                            );
                        }
            ...
            Craft.sendActionRequest('POST', 'structures/move-element', {data})
                .then(() => {
                    Craft.cp.displaySuccess(
                        Craft.t('app', 'New position saved.')
                    );
