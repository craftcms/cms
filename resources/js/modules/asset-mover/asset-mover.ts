                        ...
                        (response: any) => {
                            response.forEach((asset: any) => {
                                if (asset.error?.length) {
                                    const oldFolderId =
                                        asset.request.params.oldFolderId;

                                    // store the old folder ID of the asset that failed to move
                                    if (
                                        !folderIdsWithFailedAssets.includes(
                                            oldFolderId
                                        )
                                    ) {
                                        folderIdsWithFailedAssets.push(
                                            oldFolderId
                                        );
                                    }
                                    ...
                                    if (
                                        folderIdsToDelete.includes(oldFolderId)
                                    ) {
                                        folderIdsToDelete =
                                            folderIdsToDelete.filter(
                            ...
                            totalMoved =
                                totalMoved - folderIdsWithFailedAssets.length;
    ...
    _batchMoveRequests(requests: any, settings: any): any {
        return new Promise((resolve) => {
            ...
            this._batchRequests(requests).then((responses: any) => {
                ...
                for (const response of responses) {
                    if (response.success) {
                        if (response.newFolderId) {
                            this.undoMovedFolders.folderIds.push(
                                response.newFolderId
                            );
                ...
                Craft.elementIndex.promptHandler.showBatchPrompts(
                    (prompts: any) => {
                        Craft.elementIndex.promptHandler.resetPrompts();
                        const nextRequests = [];
                        for (const prompt of prompts) {
                            ...
                            if (settings.handleConflictChoice) {
                                const nextRequest =
                                    settings.handleConflictChoice(prompt);
                                if (
                                    prompt.request &&
                                    prompt.request.onSuccess
                                ) {
                ...
                Craft.sendActionRequest('POST', request.action, {
                    ...
                    .finally(() => {
                        ...
                        if (--activeRequests === 0) {
                            ...
                            Craft.elementIndex.progressBar.incrementProcessedItemCount(
                                1
                            );
