        ...
        <template v-for="(entryType, index) in modelValue" :key="entryType.id">
            <div
                ...
                :class="{
                    ...
                    'entry-type-item--hidden':
                        getDragState(entryType.id).type ===
            ...
            >
                ...
                <DragShadow
                    ...
                    :height="
                        getOverDropState(entryType.id)?.draggingRect?.height
                    "
                />
                ...
                <DragShadow
                    v-if="
                        getOverDropState(entryType.id)?.closestEdge === 'bottom'
                    "
                    :height="
                        getOverDropState(entryType.id)?.draggingRect?.height
                    "
    ...
    <div class="flex gap-2 mt-3 items-center">
        <craft-action-menu v-if="entryTypes?.length">
            ...
            <div slot="content">
                ...
                <template v-else>
                    <template
                        v-for="entryType in selectableTypes"
                        :key="entryType.id"
                    >
                        <craft-action-item
                            ...
                            :checked="
                                modelValue.find(
                                    (valueType) => valueType.id === entryType.id
                                )
                            "
                            :data-color="
                                (entryType.color &&
                                typeof entryType.color !== 'string'
