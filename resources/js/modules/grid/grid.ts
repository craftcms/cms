...
export class Grid extends Base<GridSettings> {
    ...
    refreshCols(force = false): void {
        ...
        if (settings.cols) {
            ...
            totalCols = Math.floor(
                this.$container.width() / settings.minColWidth
            );
    ...
    #layoutTopFill(totalCols: number): void {
        ...
        for (let item = 0; item < this.items.length; item++) {
            ...
            for (let colspan = minColspan; colspan <= maxColspan; colspan++) {
                ...
                for (
                    ...
                    this.possibleItemPositionsByColspan[item]![colspan]!.push(
                        position
                    );
                }
            }
        ...
        for (let i = 0; i < this.layouts.length; i++) {
            layoutHeights.push(
                Math.max.apply(null, this.layouts[i]!.colHeights)
            );
        }
        ...
        for (let i = 0; i < layoutHeights.length; i++) {
            if (layoutHeights[i] === shortestHeight) {
                ...
                for (let j = 0; j < totalCols; j++) {
                    emptySpace +=
                        shortestHeight - this.layouts[i]!.colHeights[j]!;
                }
        ...
        this.layout =
            shortestLayouts[
                emptySpaces.indexOf(Math.min.apply(null, emptySpaces))
            ]!;
...
class GridLayoutGenerator {
    ...
    createLayouts(
        ...
        for (let c = 0; c < grid.possibleItemColspans[item]!.length; c++) {
            ...
            const positions =
                grid.possibleItemPositionsByColspan[item]![colspan]!;
            ...
            for (let col = position; col <= endingCol; col++) {
                ...
                nextColHeights[col] =
                    tallestColHeight +
                    grid.itemHeightsByColspan[item]![colspan]!;
            }
