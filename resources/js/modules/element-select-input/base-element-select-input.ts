...
export class BaseElementSelectInput extends Base {
    ...
    canAddMoreElements(): boolean {
        return (
            ...
            (!this.settings.limit ||
                this.$elements.length < this.settings.limit)
        );
    }
                            ...
                            instances: [
                                {
                                    ...
                                    ui: [
                                        ...
                                        ? 'chip'
                                        : 'card',
                                    size: ['thumbs', 'large'].includes(
                                        this.settings.viewMode
                                    )
                                        ? 'large'
                                        : 'small',
                                    showActionMenu:
                                        this.settings.showActionMenu,
                                },
                            ],
                        },
                    ],
    ...
    removeElements($elements: any): void {
        ...
        if (this.settings.selectable) {
            ...
            $prevElement =
                lastIdx - 1 >= 0 ? this.$elements.eq(lastIdx - 1) : null;
        }
    ...
    async selectElements(elements: any[]): Promise<void> {
        ...
        for (const elementInfo of elements) {
            ...
            const $modalElement =
                elementInfo.$modalElement || elementInfo.$element;
    ...
    createNewElement(elementInfo: any): any {
        ...
        Craft.setElementSize(
            $element,
            ['thumbs', 'large'].includes(this.settings.viewMode)
                ? 'large'
                : 'small'
        );
    ...
    initSearch(): void {
        ...
        this.addListener(this.$searchInput[0], 'keydown', ((
            ev: KeyboardEvent
        ) => {
                    ...
                    'element-search/search',
                    {
                        data: {
                            ...
                            referenceElementId: this.settings.referenceElementId
                                ...
                                : null,
                            referenceElementOwnerId: this.settings
                            ...
                            referenceElementSiteId:
                                this.settings.referenceElementSiteId,
                            excludeIds,
                        },
                    }
                );
