...
export class CustomizeSourcesModal extends Modal {
    ...
    async buildModal(response: any): Promise<void> {
        ...
        if (this.multiPage) {
            ...
            $('<button/>', {
                ...
                .append(
                    $('<div class="cp-icon"/>').html(
                        await Craft.ui.icon('chevron-left')
                    )
                )
    ...
    getSourceContainer(pageName: string, create = true): any {
        if (this.sourceContainers[pageName] === undefined && create) {
            ...
            if (this.$newSourceBtn) {
                this.sourceContainers[pageName].insertBefore(
                    this.$newSourceBtn
                );
            } else {
                this.sourceContainers[pageName].appendTo(
                    this.$sourcesSidebarContent
                );
            }
    ...
    save(ev?: Event): void {
        ...
        if (
            this.multiPage &&
            this.pages!.some((page: any) => page.name === '')
        ) {
        ...
        Craft.sendActionRequest(
            ...
            .then(async ({data}: any) => {
                ...
                if (
                    ...
                    sourceKey = this.elementIndex.$visibleSources
                        .first()
                        .data('key');
                }
...
export class PageSettingsModal extends Modal {
    ...
    constructor(modal: any, nameOrSettings?: any, icon?: any, settings?: any) {
        ...
        if (
            typeof nameOrSettings === 'object' &&
            !Array.isArray(nameOrSettings)
        ) {
...
export class Source extends BaseSource {
    ...
    createViewModeField($container: any): void {
        ...
        const viewModes = this.modal.viewModes.filter(
            (viewMode: any) =>
                !viewMode.structuresOnly || this.sourceData.structureId
        );
        let defaultViewMode = this.sourceData.defaultViewMode;
        if (
            !defaultViewMode ||
            !viewModes.some(
                (viewMode: any) => viewMode.mode === defaultViewMode
            )
        ) {
    ...
    createSortField($container: any): void {
        ...
        this.sortDirectionListbox = new Craft.Listbox(
            this.$sortDirectionPicker,
            {
                onChange: ($selectedOption: any) => {
    ...
    createTableAttributesField($container: any): void {
        ...
        Craft.ui
            .createSortableCheckboxSelectField({
                ...
                values: this.sourceData.tableAttributes.map(
                    ([key]: [string]) => key
                ),
            })
            .appendTo($container);
