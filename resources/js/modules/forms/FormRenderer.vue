<script setup lang="ts">
    ...
    const effectiveErrors = computed(
        () => props.errors ?? payload.value.errors
    );
            ...
            payload.value = {
                ...
                errors: [
                    ...current.errors.filter(
                        (error) => !isWithin(error.path, scope)
                    ),
                    ...refreshed.errors,
                ],
            };
        ...
        if (focusedPath) {
            nextTick(() => {
                const control = [
                    ...
                    (element) => element.dataset.formControlPath === focusedPath
                );
                const focusTarget = control?.hasAttribute(
    ...
    function mutation(): FormPayload['values'] {
        ...
        visitControls(payload.value.nodes, (control) => {
            if (control.mode === 'editable') {
                groups.set(
                    JSON.stringify(control.deltaGroup),
                    control.deltaGroup
                );
        ...
        for (const node of nodes) {
            if (node.control) {
                ...
                node.control.forms?.forEach((form) =>
                    visitControls(form.nodes, visit)
                );
            }
...
<template>
    ...
    <template v-else>
        <ul v-if="payload.globalErrors.length" class="error-list" role="alert">
            <li v-for="error in payload.globalErrors" :key="error">
                {{ error }}
            </li>
        </ul>
        <FormNodeList
