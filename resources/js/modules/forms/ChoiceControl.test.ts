...
describe('ChoiceControl', () => {
    ...
    it('renders an icon option as an empty, labelled button', async () => {
        ...
        expect(buttons.map((b) => b.getAttribute('icon'))).toEqual([
            'asc',
            'desc',
        ]);
    ...
    it('renders the governed options checked while All is on', async () => {
        ...
        expect(items.every((item) => (item as any).checked === true)).toBe(
            true
        );
        expect(items.every((item) => (item as any).disabled === false)).toBe(
            true
        );
    });
