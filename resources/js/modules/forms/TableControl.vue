<script setup lang="ts">
    ...
    function renderTable(): void {
        ...
        rowEntries(rows).forEach(([rowId, row]) => {
            ...
            const rowElement =
                bodyElement.lastElementChild as HTMLTableRowElement;
            Object.keys(props.control.props.columns).forEach(
                (column, index) => {
    ...
    function emitRows(kind: 'discrete' | 'typing'): void {
        ...
        const entries = [
            ...(table.value?.querySelectorAll<HTMLTableRowElement>(
                'tbody > tr'
            ) ?? []),
        ].map(
            (row) =>
    ...
    function cellValue(
        ...
        if (['autosuggest', 'template'].includes(type)) {
            const combobox = cell.querySelector<
