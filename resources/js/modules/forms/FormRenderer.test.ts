...
describe('FormRenderer', () => {
    ...
    it('renders the shared payload with equivalent names, values, and errors', () => {
        ...
        expect(
            placeholder.getAttribute('aria-labelledby')?.split(/\s+/)
        ).toContain(placeholderLabel.id);
        expect(placeholderLabel.textContent).toContain('Placeholder Text');
        expect(
            placeholderField.querySelector(':scope > [slot="help-text"]')
                ?.textContent
        ).toBe(
            'The text that will be shown if the field doesn’t have a value.'
        );
        expect(
            placeholderField.querySelector(':scope > [slot="feedback"]')
                ?.textContent
        ).toContain('Placeholder is invalid.');
        expect(mode.value).toBe('enlarged');
        expect(
            container.querySelector('craft-field craft-input')
        ).not.toBeNull();
        expect(
            container.querySelector('craft-field craft-select')
        ).not.toBeNull();
        expect(
            container.querySelector('craft-field craft-switch')
        ).not.toBeNull();
        ...
        ).toContain('The settings could not be saved.');
    });
    ...
    it('renders table cell errors beside scalar values', async () => {
        ...
        expect(
            inputs.map((input) =>
                input.closest('td')?.classList.contains('error')
            )
    ...
    it('uses a path slot to override a control without replacing Form behavior', async () => {
        ...
        expect(
            container.querySelector('[slot="feedback"]')?.textContent
                        ...
                        'button',
                        {
                            ...
                            onClick: () => {
                                if (!Array.isArray(slot.value)) {
                                    throw new Error(
                                        'Expected previewTargets to be an array.'
                                    );
                                }
                ...
                if (url === 'fields/render-condition-builder') {
                    return {
                        data: {
                            ...
                            bodyHtml:
                                '<script data-condition-builder></script>',
                        },
                    };
                }
    ...
    it('renders FieldLayout tabs and semantic content', async () => {
                    ...
                    children: [
                        {
                            ...
                            props: {
                                ...
                                warningHtml:
                                    'This appears <strong>publicly</strong>.',
                                required: true,
        ...
        expect(
            statusField?.shadowRoot?.querySelector(
                '.form-field__status-indicator'
            )
        ).not.toBeNull();
        expect(tab?.querySelector('[slot="tip"] em')?.textContent).toBe(
            'sentence'
        );
        ...
        expect(
            tab?.querySelector('hr[data-form-node="separator"]')
        ).not.toBeNull();
        expect(
    ...
    it('renders server-localized copy unchanged', async () => {
        ...
        required(
            localized.nodes[0],
            'Expected the first field node.'
        ).props.label = 'UI-Modus';
        app.unmount();
        ...
        missing.nodes = [
            {
                ...
                props: {
                    ...
                    error: 'Form Node provider [Acme\\Forms\\MissingLayoutElement] is unavailable.',
            ...
            },
            {
                ...
                control: {
                    ...
                    props: {
                        ...
                        error: 'Form Control provider [Acme\\Forms\\MissingField] is unavailable.',
    ...
    it('keeps the active text expander suggestion across a form refresh', async () => {
        ...
        await vi.waitFor(() =>
            expect(initialOptions[0]!.getAttribute('aria-selected')).toBe(
                'true'
            )
        );
    ...
    it('ignores stale refreshes and retains the last valid presentation on failure', async () => {
        ...
        required(requests[0], 'Expected the stale refresh request.').resolve(
            stale
        );
        await Promise.resolve();
        await nextTick();
    ...
    it('retains removed values without including them in submissions', async () => {
        ...
        expect(renderer.currentValues()).not.toHaveProperty(
    ...
    it('submits complete atomic groups when one member changes', async () => {
        ...
        if (
            !(atomicSettings instanceof Object) ||
            Array.isArray(atomicSettings)
        ) {
            throw new Error('Expected settings values to be an object.');
        }
    ...
    it('selects and removes ordered element relationships as changed-only values', async () => {
        ...
        const selectElements = async (
            ...
            for (let i = 0; i < 20; i++) {
                if (
                    selectorModal.createElementSelectorModal.mock.calls.length >
                    0
                ) {
                    break;
                }
        ...
        container
            .querySelector<HTMLElement>('[data-element-select-add]')!
            .click();
    ...
    it('renders and submits scalar and choice Controls', async () => {
        ...
        const controls: Array<
            [string, string, string, FormProperties, FormValue]
        > = [
            [
                'craft:text',
                'Text',
        ...
        for (const mode of ['readOnly', 'disabled'] as const) {
            ...
            expect(
                container.querySelector('craft-input-color')?.modelValue
            ).toBe('ff0000');
        }
    });
        ...
        const locality = required(
            selectLocalityFields.find(
                (field) =>
                    ...
                    Object.getOwnPropertyDescriptor(field, 'name')?.value ===
                        'locality'
            ),
    ...
    it('edits nested Controls as one ordered atomic mutation', async () => {
        ...
        const firstForm = required(
            required(
                required(nested.nodes[0], 'Expected the Matrix field node.')
                    .control,
        ...
        const refresh = vi.fn(
            async (_values: FormPayload['values'], scope?: string[]) => {
                if (!scope)
                    throw new Error('Expected the nested refresh scope.');
        ...
        expect(container.querySelectorAll('[data-content-block]')).toHaveLength(
            1
        );
    ...
    it('renders a just-added entry against the Form the server scoped to its UUID', async () => {
        ...
        expect(
            container.querySelector('.matrixblock craft-spinner')
        ).toBeNull();
        expect(
            container.querySelector(
