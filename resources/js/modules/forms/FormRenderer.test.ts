...
describe('FormRenderer', () => {
    ...
    it('accepts override values derived from reactive arrays', async () => {
        ...
        container
            .querySelector<HTMLButtonElement>('[data-add-target]')!
            .click();
        await nextTick();
    ...
    it('invalidates the complete Form when a payload component is not registered', async () => {
        ...
        expect(
            container.querySelector('[role="alert"]')?.textContent
        ).toContain(invalid.nodes[0]!.type);
    ...
    it('retains removed values without including them in submissions', async () => {
        ...
        (refreshed.values.settings as Record<string, unknown>).uiMode =
            'normal';
    ...
    it('drops unsaved values inside nested Forms when the host resets', async () => {
        ...
        nested.values = {
            settings: {
                matrix: {
                    entries: {
                        'block-a': {type: 'text', heading: 'Canonical heading'},
                    },
                    sortOrder: ['block-a'],
                },
            },
    ...
    it('renders and submits scalar and choice Controls', async () => {
        const controlsPayload = structuredClone(
            payload
        ) as Mutable<FormPayload>;
        const controls = [
            [
                'craft:text',
                'Text',
        ...
        controlsPayload.nodes = controls.map(
    ...
    it('renders and submits composite and rich Controls', async () => {
        ...
        await vi.waitFor(() =>
            expect(
                (renderer.currentValues().settings as Record<string, unknown>)
                    .rows
            ).toEqual([
                {name: '<Row>', enabled: true},
                {name: '', enabled: false},
    ...
    it('edits nested Controls as one ordered atomic mutation', async () => {
        ...
        const clearContent = [
            ...container.querySelectorAll('craft-button'),
        ].find((button) => button.textContent?.includes('Clear content'))!;
        clearContent.click();
        await nextTick();
        expect(renderer.currentValues()).toMatchObject({
            settings: {
    ...
    it.each(['readOnly', 'disabled'] as const)(
        ...
        async (mode) => {
            const nonEditable = structuredClone(
                payload
            ) as Mutable<FormPayload>;
            visitControls(
                nonEditable.nodes,
    ...
    for (const node of nodes) {
        if (node.control) {
            ...
            node.control.forms?.forEach((form) =>
                visitControls(
                    form.nodes as Mutable<FormPayload>['nodes'],
                    visit
                )
            );
        }
