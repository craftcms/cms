<script setup lang="ts">
    ...
    const providedPresentations = computed(
        () =>
            new Map(
                props.control.props.elements.map((element) => [
                    element.id,
                    element,
                ])
            )
    ...
    function selected(event: Event): void {
        const elements = (event as CustomEvent<{elements?: ElementInfo[]}>)
            ?.detail?.elements;
    ...
    function sync(): void {
        ...
        if (
            ...
            selected.every(
                (value, index) => value === Number(props.value[index])
            )
        ) {
            return;
        }
    ...
    <div>
        ...
        >
            <ul class="elements chips chips-small">
                <li
                    v-for="selectedValue in value"
                    :key="elementId(selectedValue)"
                >
                    <craft-chip
                        class="element"
                        size="small"
                        ...
                        :data-site-id="
                            presentation(selectedValue).siteId ?? undefined
                        "
                    >
