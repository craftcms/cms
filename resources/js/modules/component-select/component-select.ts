...
export class ComponentSelect extends Base<ComponentSelectSettings> {
    ...
    #addChipActions(chip: HTMLElement): void {
        ...
        this.container.dispatchEvent(
            new CustomEvent<DefineChipActionsEventDetail>(
                'define-chip-actions',
                {
                    bubbles: true,
                    detail: {chip, actions},
        ...
        if (this.#menu) {
            actions.push({
                ...
                callback: () => {
                    ...
                    // instance lookup.
                    chip.dispatchEvent(
                        new CustomEvent<ReplaceComponentEventDetail>(
                            'replace-component',
                            {
                                bubbles: true,
                                cancelable: true,
    ...
    #chipDisclosureMenu(chip: HTMLElement): any {
        ...
        if (actionMenu) {
            const container =
                actionMenu.querySelector<HTMLElement>('[slot="content"]') ??
                actionMenu;
    ...
    #initDragSort(): void {
        ...
        this.#dragSort = new DragSort({
            ...
            axis: this.#list?.classList.contains('inline-chips')
                ? null
                : Y_AXIS,
    ...
    #draggeeFilter(): HTMLElement[] {
        ...
        if (chip && this.#select?.isSelected(chip)) {
            return this.#select
                ...
                .filter(
                    (li: HTMLLIElement | null): li is HTMLLIElement =>
                        li !== null
                );
        }
    ...
    #initReorderButton(li: HTMLElement): void {
        ...
        btn.addEventListener('reorder', (event: Event) => {
            const {direction} = (
                event as CustomEvent<{direction: ReorderDirection}>
            ).detail;
    ...
    #getOption(id: string | number): HTMLElement | null {
        const key = String(id);
        return (
            this.#options().find((option) => option.dataset.id === key) ?? null
        );
    }
    ...
    #addMenuOption(
        ...
        if (this.settings.showHandles && handle) {
            ...
            handleEl.className =
                'menu-item-description mt-2xs smalltext light code';
    ...
    #canAddMore(): boolean {
        return (
            !this.settings.limit || this.#chips().length < this.settings.limit
        );
    }
    ...
    #focusAfterRemoval(nextLi: HTMLElement | null): void {
        const target =
            ...
            (this.#canAddMore() && this.#addBtn
                ? this.#addBtn
                : this.#chipActionButton(
                      this.#chips().pop()?.closest('li') ?? null
                  ));
