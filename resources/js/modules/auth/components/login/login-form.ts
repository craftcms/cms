...
export default class CraftLoginForm extends LitElement {
        ...
        try {
            const response = await fetch(this.action, {
                ...
                headers: {
                    ...
                    'X-CSRF-TOKEN':
                        ConfigService.getInstance().get('csrfTokenValue'),
                },
                body: JSON.stringify({
    ...
    #onResetBack(event: CustomEvent) {
        ...
        this.updateComplete.then(() => {
            if (username && this._usernameInput)
    ...
    #renderLoginView() {
        const hasAltMethods =
            this._canUsePasskey ||
            this.querySelector('[slot="alternative-methods"]');
