...
export class MatrixEntry extends Base {
    ...
    constructor(matrix: MatrixInput, container: HTMLElement) {
        ...
        this.isNew =
            !this.id ||
            (typeof this.id === 'string' && this.id.startsWith('new'));
        ...
        if (actionMenuBtn) {
            this.actionDisclosure =
                (jqData(
                    actionMenuBtn,
                    'disclosureMenu'
                ) as LegacyDisclosureMenu) ||
                new (legacyGarnish().DisclosureMenu)(actionMenuBtn);
            this.actionMenu =
                (this.actionDisclosure.$container as {0?: HTMLElement})?.[0] ??
                null;
        }
        ...
        setTimeout(() => {
            this.formObserver = new (craft().FormObserver)(
                container,
                (data) => {
    ...
    private actionMenuOptions(): HTMLElement[] {
        return Array.from(
            this.actionMenu?.querySelectorAll<HTMLElement>(
                'button[data-action]'
            ) ?? []
        );
    }
    ...
    private prepareActionMenu(): void {
        ...
        hideActions.push(
            this.container.classList.contains('disabled-entry')
                ? 'disable'
                : 'enable'
        );
        ...
        const labels: Record<string, string> = {
            ...
            duplicate: bulk
                ...
                : t('Duplicate'),
            copy: bulk
                ? t('Copy selected {type}', {type: t('blocks')})
                : t('Copy'),
            delete: bulk
        ...
        if (pasteBtn) {
            ...
            if (showPasteButton) {
                ...
                const label = pasteBtn.querySelector(
    ...
    previewHtml(): string {
        ...
        const fields = Array.from(
            this.fieldsContainer?.querySelectorAll<HTMLElement>(
                ':scope > * > *'
            ) ?? []
        );
    ...
    private inputPreviewText(input: HTMLElement): unknown {
        if (input instanceof HTMLSelectElement) {
            return Array.from(input.selectedOptions).map(
                (option) => option.text
            );
        }
        ...
        switch (option.getAttribute('data-action')) {
            ...
            case 'copy': {
                const entries = this.bulkActionMode()
                    ? Array.from(
                          this.matrix.entrySelect?.getSelectedItems() ?? []
                      )
                ...
                craft().cp.copyElements(
                    entries.map((entry) => ({
                        ...
                        id:
                            entry.matrix.elementEditor?.getDraftElementId(
                                entry.id
                            ) || entry.id,
    ...
    updateFieldLayout(data: string): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            const param = (n: string) =>
                craft().namespaceInputName(n, baseInputName);
            const extraData: Record<string, unknown> = {
                ...
                [param('sortOrder')]:
                    Array.from(
                        this.container.parentElement?.children ?? []
                    ).indexOf(this.container) + 1,
                [param('typeId')]: this.container.dataset.typeId,
                [param('elementUid')]:
                    elementEditor?.getDraftElementUid(
                        this.container.dataset.uid
                    ) ?? this.container.dataset.uid,
            };

            const selectedTab =
                this.fieldsContainer?.querySelector<HTMLElement>(
    ...
    private async afterUpdateFieldLayout(
        ...
        for (const tabInfo of response.data.missingElements) {
            ...
            if (!tabContainer) {
                ...
                tabContainer.id = craft().namespaceId(
                    tabInfo.id,
                    baseInputName
                );
            ...
            for (const elementInfo of tabInfo.elements) {
                if (elementInfo.html !== false) {
                    (visibleLayoutElements[tabInfo.uid] ??= []).push(
                        elementInfo.uid
                    );

                    if (elementInfo.static) {
                        (staticLayoutElements[tabInfo.uid] ??= []).push(
                            elementInfo.uid
                        );
                    }
        ...
        if (this.hasTabs && this.tabContainer) {
            this.tabContainer.insertAdjacentHTML(
                'beforeend',
                response.data.tabs!
            );
            this.tabManager = MatrixEntry.initTabs(this.tabContainer);
