...
export class MatrixInput extends Base<MatrixInputSettings> {
    ...
    constructor(
        ...
        this.addEntryBtn =
            this.addEntryBtnContainer?.querySelector('.btn:not(.menubtn)') ??
            null;
        this.addEntryMenuBtns = Array.from(
            this.addEntryBtnContainer?.querySelectorAll<HTMLElement>(
                '.menubtn'
            ) ?? []
        );
        this.statusMessage = this.container.querySelector(
            '[data-status-message]'
        );
        ...
        if (this.settings!.formControl || craft().hasMousePointerEvents()) {
            this.entrySort = new DragSort(entries, {
                ...
                filter: () => {
                    ...
                    if (
                        this.entrySort?.$targetItem?.classList.contains(
                            'sel'
                        ) &&
                        this.entrySelect
                    ) {
        ...
        for (const btn of this.settings!.formControl
            ? []
            : this.addEntryMenuBtns) {
            ...
            for (const menuBtn of menuContainer?.querySelectorAll<HTMLElement>(
                ...
                this.addListener(menuBtn, 'activate', async (event) => {
                    ...
                    try {
                        await this.addEntry(
                            (ev.currentTarget as HTMLElement).getAttribute(
                                'data-type'
                            ) ?? ''
                        );
                    } finally {
        ...
        const finishInit = (
            elementEditor: LegacyElementEditor | null
        ): void => {
    ...
    canAddMoreEntries(num = 1): boolean {
        ...
        return (
            !this.maxEntries ||
            (this.entriesContainer?.children.length ?? 0) + num <=
                this.maxEntries
        );
    }
            ...
            try {
                ...
                requestAnimationFrame(() => {
                    if (autofocus) {
                        ...
                        const fields =
                            entry.querySelector<HTMLElement>('.flex-fields');
                        const focusable = fields
                            ? firstFocusableElement(fields)
                            : null;
                        if (
                            focusable &&
    ...
    private async addDefaultEntries(
        type: string,
        count: number
    ): Promise<void> {
        if (this.elementEditor) {
    ...
    callOnSelectedEntries(
        ...
        for (const item of Array.from(
            this.entrySelect?.getSelectedItems() ?? []
        )) {
