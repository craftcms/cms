...
import { withNavCrumbMenus, withSubnavCrumbs } from "@/common/composables/subnavCrumbs";
...
const FOOTER_SLOTS = ["content-footer", "additional-buttons", "submit-button"] as const;
const SIDEBAR_SLOTS = ["content-sidebar", "subnav-actions"] as const;

const headerSlots = computed(() => HEADER_SLOTS.filter((name) => slots[name]));
const footerSlots = computed(() => FOOTER_SLOTS.filter((name) => slots[name]));
const sidebarSlots = computed(() => SIDEBAR_SLOTS.filter((name) => slots[name]));
...
const hasFooter = computed(
  () => Boolean(props.form) || regions.has("content-footer") || regions.has("additional-buttons"),
);
...
const detailsOverlaid = useDetailsOverlay(() => contentLayout.value, contentLayoutWidth);
...
<template>
  ...
  <ScreenSkipLinks :has-sidebar="hasSidebar" :additional-skip-links="additionalSkipLinks" />
              ...
              <main id="main" tabindex="-1">
                <form method="post" @submit.prevent="form && save()" class="cp-main">
                  ...
                  >
                    ...
                    <div id="content-main" class="cp-content__main" :style="contentMainStyle">
                      <div
                        :class="{
                    ...
                    <aside v-show="hasDetails" class="cp-content__details">
                      <div class="sticky top-0 h-screen">
                        <ContentDetails ref="detailsColumn" :resizer="detailsResizer">
...
.cp-content {
  ...
  --cp-content-details-width: clamp(var(--cp-content-details-rail), 90vw, calc(400rem / 16));
