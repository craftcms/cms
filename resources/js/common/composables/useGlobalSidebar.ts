 ...
 *
 * Every caller gets the *same* sidebar — the shell sizes its grid column from
 * it while the sidebar itself renders the toggle, so built per call each would
 * get its own copy and toggling would move one while the other kept rendering
 * the old state. `createGlobalState` runs this factory once, inside a detached
 * effect scope, and hands every later caller the same result.
 *
 * Running on first call rather than at module scope also matters:
 * `useLocalStorage` prefixes its key with `Craft.systemUid`, and the CP config
 * doesn't set that until the app boots — later than this module is evaluated.
 */
export const useGlobalSidebar = createGlobalState((): GlobalSidebar => {
  const sidebar = reactive<GlobalSidebarState>({
    mode: 'floating',
    visibility: 'hidden',
  });
   ...
   * on close. Registered by whichever component renders it — a `useTemplateRef`
   * out here would resolve against no template at all.
   */
  ...
  // rendered the sidebar and had to render it already collapsed; this one is
  // rendered by Vue, and with Inertia SSR off the preference is read before the
  ...
  // Only remember what the user chose for the docked sidebar. A floating one is
  // hidden because the window is narrow, not because anyone asked for it, and
  // storing that would expand the rail on the next wide-screen visit.
  watch(
    () => sidebar.visibility,
    (visibility) => {
