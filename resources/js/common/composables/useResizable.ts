...
export function useResizable(options: UseResizableOptions): UseResizableReturn {
  ...
  const {width: measuredWidth} = useElementSize(() => toValue(options.target));
  ...
  function setWidth(
    ...
    if (!clamp) {
      ...
      lastDetail = buildDetail(Math.round(next), Math.round(next), source);
  ...
  watch(handle, (el) => {
    ...
    dragger = new BaseDrag(el, {
      ...
      onBeforeDragStart: () => {
        ...
        options.onResizeStart?.(buildDetail(startWidth, startWidth, 'pointer'));
      },
      onDrag: () => {
      ...
      onDragStop: () => {
        endResize();
        options.onResizeEnd?.(
          lastDetail ?? buildDetail(currentWidth(), currentWidth(), 'pointer')
        );
      },
    });
  });
