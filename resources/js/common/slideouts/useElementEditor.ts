...
export function useElementEditor(
  ...
  function tabsRoot(): HTMLElement | null {
    return (
      toValue(options.regions.tabs)?.querySelector<HTMLElement>('.pane-tabs') ??
      null
    );
  }
  ...
  function initTabManager(): void {
    ...
    tabManager.on('selectTab', (ev: any) => {
      $(document.querySelector(ev.$tab.attr('href'))).removeClass('hidden');
            ...
            ? Craft.t(
                ...
                ($tabsWithErrors.length > 1
                  ? `<span class="visually-hidden">${Craft.t(
                      ...
                      {total: errorCount, num: $tabsWithErrors.length}
                    )}</span>`
                  : '')
