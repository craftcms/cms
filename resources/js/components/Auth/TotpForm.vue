<script setup lang="ts">
  ...
  async function handleInlineSubmit() {
    ...
    const url = props.action ?? TwoFactorAuthenticationController.verify['/admin/actions/auth/verify-totp']().url;

    try {
      const response = await http.post(url);
      emit('success', response);
    } catch (e: any) {
      error.value = e?.response?.data?.message ?? t('Invalid verification code.');
      emit('error', e);
      shaking.value = true;
      setTimeout(() => {
        shaking.value = false;
...
<template>
  ...
  <template v-else>
    <Form :action="action" #default="{processing, errors}" method="post">
      <div class="flex gap-2 items-end">
        <input type="hidden" name="redirect" :value="returnUrl" v-if="returnUrl" />
        <CraftInput
          :label="t('Verification Code')"
          id="verification-code"
          name="code"
