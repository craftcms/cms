...
function generatePropertyValueWrapper(component) {
  const modelTypeImport = component.modelTypeImport
    ? `\n  import type {${component.modelTypeImport}} from '${component.importPath}.ts.mjs';`
    : '';
...
<script setup lang="ts">
  ...
  function onInput(event: Event) {
    ...
    model.value = (event.target as ${component.className}).value;
  }
</script>

<template>
  <${component.tagName} .value="model" @input="onInput">
    <slot></slot>
  </${component.tagName}>
</template>
`;
