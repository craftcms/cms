...
/*
  Also on [data-theme], not just :root. These tokens are built out of the raw
  palette with var(), and a custom property carrying a var() is substituted
  where it is declared -- so resolving them only at :root bakes in the root's
  palette, and re-declaring the palette further down can never reach back into
  them. Repeating the block on a themed element re-resolves it against that
  element's palette, which is what makes a themed subtree possible.
*/
:root,
[data-theme] {
