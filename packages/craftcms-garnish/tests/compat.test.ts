...
describe('Listbox compat (conditional)', () => {
  const Listbox = (Core as AnyObj).Listbox;
  const maybe = Listbox ? it : it.skip;
  ...
  maybe('exposes an extend-able Garnish.Listbox', () => {
    const G = GarnishCompat as AnyObj;
    expect(typeof G.Listbox).toBe('function');
    expect(typeof G.Listbox.extend).toBe('function');

    const container = makeContainer();
    const lb = new G.Listbox(container);
    expect(lb).toBeInstanceOf(Listbox);
  });

  maybe('hands a legacy onChange a jQuery-wrapped option', () => {
    // Install a minimal fake jQuery so the compat layer wraps with $().
    const fakeProto = {};
    function FakeJq(this: any, value?: unknown): any {
      const coll: any = Object.create(fakeProto);
      coll.length = value ? 1 : 0;
      if (value) {
        coll[0] = value;
      }
      coll.__isFakeJq = true;
      return coll;
    }
    FakeJq.prototype = fakeProto;
    (FakeJq as any).fn = {jquery: '3.x'};
    ...
    try {
      ...
      const lb = new G.Listbox(container, {
        onChange(option: any, index: number) {
          received.push(option, index);
        },
      });
      lb.select(1);
      // The first arg is a (fake) jQuery collection wrapping the option element.
      expect((received[0] as any).__isFakeJq).toBe(true);
      expect((received[0] as any)[0]).toBe(lb.$options[1]);
