 ...
 * CP Screen Slideout
 *
 * Defined lazily: `Craft.Slideout` is now assigned by a Vite-bundled module
 * (`resources/js/modules/slideout`), and this legacy webpack bundle isn't
 * guaranteed to execute after that Vite module has run — the two bundles load
 * independently, so evaluating `Craft.Slideout.extend(...)` eagerly here (as
 * this file used to) could run before `Craft.Slideout` exists. A lazy getter
 * defers the `.extend()` call (and the `Craft.Slideout` lookup) until
 * something actually reads `Craft.CpScreenSlideout` — by the time any CP code
 * does that (constructing a slideout in response to user interaction), every
 * entrypoint's modules have finished loading. On first access the getter
 * builds the extended class, replaces itself with a plain cached value, and
 * returns it.
 */
Object.defineProperty(Craft, 'CpScreenSlideout', {
  configurable: true,
  get() {
    const CpScreenSlideout = Craft.Slideout.extend(
      {
        action: null,
        ...
        init: function (action, settings) {
          ...
          settings = Object.assign(
            {},
            Craft.CpScreenSlideout.defaults,
            settings
          );
          ...
          this.$toolbar = $('<div/>', {class: 'so-toolbar'}).appendTo(
            this.$header
          );
          ...
          // Content
          this.$content = $('<div/>', {class: 'so-content'}).appendTo(
            this.$body
          );
          ...
          if (this.tabManager !== null) {
            ...
            $tabs.each(function (i, tab) {
              ...
              if ($tabContainer.length > 0) {
                ...
                let tabErrorCount =
                  $tabErrorSummary.find('ul.errors li').length;
                let headingText = '';
    ...
    Object.defineProperty(Craft, 'CpScreenSlideout', {
      configurable: true,
      writable: true,
      value: CpScreenSlideout,
    });

    return CpScreenSlideout;
  },
});
